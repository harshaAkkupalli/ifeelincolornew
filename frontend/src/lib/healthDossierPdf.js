...
export async function generateHealthDossier({
  ...
  // Route through downloadBlob so the file saves in browser, PWA, Capacitor
  // APK (→ @capacitor/filesystem) and other WebView wrappers alike.
  const blob = pdf.output('blob');
  // Lazy-import to avoid a cycle if healthDossierPdf is loaded before platform.
  const { downloadBlob } = await import('./platform');
