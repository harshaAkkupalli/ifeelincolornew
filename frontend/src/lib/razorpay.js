...
export function useRazorpayCheckout() {
  ...
  // Reload config every 30 s so changes pushed from the Admin Payment-Setup
  // page (e.g. toggling Razorpay ON) are reflected without the user having
  // to refresh their session.
  useEffect(() => {
    let alive = true;
    const fetchConfig = () => axios.get(`${API}/razorpay/config`)
