...
export default function OrganizationDashboard() {
  ...
  const load = async () => {
    try {
      ...
      // Pull announcements that target organisations (or everyone).
      try {
        const ar = await ax.get(`${API}/announcements/active`);
        const items = (ar.data?.announcements || []).filter((a) => {
          const aud = a.target_audience || a.audience || 'all';
          return aud === 'organization' || aud === 'all' || aud === 'everyone';
        });
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #FFF8EE 0%, #FFFFFF 100%)' }} data-testid="org-dashboard">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        {/* Announcements — auto-scroll carousel (org-targeted + 'everyone') */}
        {announcements.length > 0 && (
          <div className="mb-6" data-testid="org-announcements-section">
            <p className="text-xs font-nunito font-bold uppercase tracking-widest mb-2 text-slate-500">
              Announcements
            </p>
            <AnnouncementsRail
