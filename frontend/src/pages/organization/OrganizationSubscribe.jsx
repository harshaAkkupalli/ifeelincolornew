  ...
  const handleSubscribe = (plan) => {
    ...
    launchCheckout({
      ...
      onSuccess: () => {
        ...
        setPaymentResult({
          presetKey: 'success',
          title: `${plan.name} is live for your organization 🏢`,
          primaryLabel: 'Open dashboard',
          onPrimary: () => { setPaymentResult(null); navigate('/org/dashboard'); },
        });
      },
      onFailure: (e) => {
        setBusyPlanId(null);
        if (e?.dismissed) {
          setPaymentResult({
            presetKey: 'dismissed',
            onPrimary: () => { setPaymentResult(null); handleSubscribe(plan); },
          });
        } else {
          setPaymentResult({
            presetKey: 'failure',
            message: e?.message
              ? `No funds were captured. (${e.message}) Please retry, or contact the IFEELINCOLOR billing team if the issue persists.`
              : undefined,
            onPrimary: () => { setPaymentResult(null); handleSubscribe(plan); },
          });
        }
                ...
                >
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-2">
                    <Button
                      ...
                      className="text-white border-0 rounded-xl text-xs h-10"
                      style={{
                        background: isCurrent
                          ? '#E2E8F0'
                          : (featured ? 'linear-gradient(135deg, #10B981, #059669)' : 'linear-gradient(135deg, #6366F1, #4F46E5)'),
                        color: isCurrent ? '#94A3B8' : 'white',
                        opacity: (!configured && !isCurrent) ? 0.5 : (isCurrent ? 0.7 : 1),
                        cursor: isCurrent ? 'not-allowed' : 'pointer',
                      }}
                    >
                      {busyPlanId === plan.plan_id ? 'Opening…' : 'Subscribe'}
                    </Button>
                    <Button
                      disabled={!isCurrent}
                      data-testid={`unsubscribe-btn-${plan.plan_id}`}
                      onClick={async () => {
                        if (!isCurrent) return;
                        if (!window.confirm('Cancel your organization subscription? Your portal access remains until the end of the billing period.')) return;
                        try {
                          await ax.post(`${API}/me/subscription/cancel`);
                      ...
                      className="rounded-xl text-xs h-10 border"
                      style={{
                  ...
                  {isCurrent && (
                    <p className="text-[9px] text-emerald-700 font-semibold uppercase tracking-wider mt-2 text-center">
                      ✓ Current plan
                    </p>
                  )}
                </motion.div>
