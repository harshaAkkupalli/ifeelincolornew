...
import { PATIENT_ONBOARDING } from '../lib/onboardingScreens';
...
export default function PatientSignup() {
  ...
  // Replay the onboarding carousel before the multi-step signup form
  // so users land on a value-first surface (key for marketing).
  const [showOnboarding, setShowOnboarding] = useState(() => {
    if (typeof window === 'undefined') return false;
    return !window.localStorage.getItem('ifc_onboard_seen_patient');
  });
  ...
  const finishOnboarding = () => {
    try { window.localStorage.setItem('ifc_onboard_seen_patient', '1'); } catch { /* ignore */ }
  ...
  return (
    ...
    {!showSplash && showOnboarding && (
      <OnboardingCarousel
        screens={PATIENT_ONBOARDING}
        intervalMs={6000}
        onFinish={finishOnboarding}
        testidPrefix="onboarding-patient-signup"
      />
    )}
