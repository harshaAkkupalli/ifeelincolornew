...
import { Megaphone, MapPin, Heart, Sparkles, AlertOctagon, Star, ChevronRight, Plus, X, Check, Lock as LockIcon, BookOpen, ScanFace, Loader2, History, Building2, ExternalLink, Search, Share2, MessageSquare } from 'lucide-react';
...
export default function PatientHomeScreen() {
  ...
  const [doctorTab, setDoctorTab] = useState('nearby'); // nearby | subscribed | all
  const [allClinicians, setAllClinicians] = useState([]);
  const [maskedModal, setMaskedModal] = useState(null);  // clinician obj to show paywall popup
  const [modalDoctor, setModalDoctor] = useState(null);
  const [showCheckin, setShowCheckin] = useState(false);
  const [progress, setProgress] = useState({});
  const [showWelcome, setShowWelcome] = useState(false);
  const [paymentMsg, setPaymentMsg] = useState('');
  // Rating-modal state for the "My Subscribed" tab.
  const [ratingModalDoc, setRatingModalDoc] = useState(null); // doctor obj
  const [ratingValue, setRatingValue] = useState(0);
  const [ratingFeedback, setRatingFeedback] = useState('');
  // Search bars (Nearby + All Clinicians)
  const [nearbySearch, setNearbySearch] = useState('');
  const [allClinSearch, setAllClinSearch] = useState('');
  const [notFoundModal, setNotFoundModal] = useState(null); // { name } when search misses
  ...
  const loadAllClinicians = async () => {
    try {
      const r = await axios.get(`${API}/clinicians/discover`, { withCredentials: true });
      setAllClinicians(r.data.clinicians || []);
    } catch { /* ignore */ }
  };

  // Eagerly load the master clinician list so the "All Clinicians" tab
  // count is accurate from the first render (without requiring the user
  // to click the tab first).
  useEffect(() => { loadAllClinicians(); }, []);

  // Auto-refresh subscribed clinicians + master list every 12 s so a new
  // subscription / unsubscribe done from another tab (or by a clinician)
  // appears here without a manual reload (user directive 2026-06-04).
  useEffect(() => {
    const iv = setInterval(() => {
      refreshSubscribed();
      if (doctorTab === 'all') loadAllClinicians();
    }, 12000);
    return () => clearInterval(iv);
  }, [doctorTab]); // eslint-disable-line

  useEffect(() => {
    if (doctorTab === 'all' && allClinicians.length === 0) {
      loadAllClinicians();
    }
  }, [doctorTab]); // eslint-disable-line

  useEffect(() => {
    // Only show announcements that target patients explicitly OR target
    // 'all' / 'everyone'. Without this filter, an announcement created for
    // clinicians / organizations would also leak into the patient home.
    axios.get(`${API}/announcements/active`).then((r) => {
      const items = (r.data.announcements || []).filter((a) => {
        ...
        return aud === 'patient' || aud === 'all' || aud === 'everyone';
      });
  ...
  // Filtered views driven by the new search bars (#3).
  const nearbyFiltered = React.useMemo(() => {
  ...
  const allClinFiltered = React.useMemo(() => {
    const q = allClinSearch.trim().toLowerCase();
    if (!q) return allClinicians;
    return allClinicians.filter(c => (c.name || '').toLowerCase().includes(q));
  }, [allClinicians, allClinSearch]);

  // Submit on All Clinicians search → if no match, show "not on platform" popup.
  const submitAllClinSearch = () => {
    const q = allClinSearch.trim();
    if (!q) return;
    if (allClinFiltered.length === 0) setNotFoundModal({ name: q });
  };

  // Open the rating modal for a clinician (My Subscribed tab).
  const openRatingModal = async (doc) => {
    setRatingModalDoc(doc);
    setRatingValue(0);
    setRatingFeedback('');
    try {
      const r = await axios.get(`${API}/patient/doctors/${doc.doctor_id}/rating`, { withCredentials: true });
      if (r.data?.rating) setRatingValue(r.data.rating);
      if (r.data?.feedback) setRatingFeedback(r.data.feedback);
    } catch { /* fresh rating */ }
  };
  const submitRating = async () => {
    if (!ratingModalDoc || ratingValue < 1) return;
    try {
      await axios.put(
        `${API}/patient/doctors/${ratingModalDoc.doctor_id}/rating`,
        { rating: ratingValue, feedback: ratingFeedback },
        { withCredentials: true },
      );
      setPaymentMsg(`Thanks for rating ${ratingModalDoc.name}!`);
      setTimeout(() => setPaymentMsg(''), 2500);
      setRatingModalDoc(null);
    } catch (err) {
      setPaymentMsg(err?.response?.data?.detail || 'Could not save rating');
      setTimeout(() => setPaymentMsg(''), 2500);
    }
  };
  // Quick unsubscribe inside the My Subscribed tab.
  const unsubscribeDoctor = async (doc) => {
    if (!window.confirm(`Cancel your subscription to ${doc.name}?`)) return;
    try {
      await axios.delete(`${API}/patient/doctors/subscribe/${doc.doctor_id}`, { withCredentials: true });
      setPaymentMsg(`Unsubscribed from ${doc.name}`);
      await refreshSubscribed();
      await loadAllClinicians();
      setTimeout(() => setPaymentMsg(''), 2500);
    } catch (err) {
      setPaymentMsg(err?.response?.data?.detail || 'Could not unsubscribe');
      setTimeout(() => setPaymentMsg(''), 2500);
    }
  };
  // Share invite to a clinician who isn't on IFEELINCOLOR yet (popup CTA).
  const shareInvite = (name) => {
    const text = `Hi ${name || ''}, I've been using IFEELINCOLOR — a fantastic mental-health platform — and would love to subscribe to you on it. Join here: ${window.location.origin}`;
    if (navigator.share) {
      navigator.share({ title: 'Join IFEELINCOLOR', text }).catch(() => {});
    } else if (navigator.clipboard) {
      navigator.clipboard.writeText(text).then(() => {
        setPaymentMsg('Invite copied to clipboard');
        setTimeout(() => setPaymentMsg(''), 2500);
      });
    }
  };
  ...
  return (
    <div className="px-5 pt-5 pb-6 relative" style={{
      ...
      <section className="mt-6 relative" data-testid="patient-doctors-section">
        ...
        <div className="flex items-center gap-2 mb-3 overflow-x-auto no-scrollbar">
          {[
            ...
            { key: 'all', label: 'All Clinicians', count: allClinicians.length },
          ].map(t => {
            ...
            return (
              <button
                ...
                className="shrink-0 rounded-2xl px-3 py-2.5 font-nunito font-bold text-xs transition-all relative"
                style={{
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {doctorTab === 'nearby' && (
              <>
                {/* Search bar — autocomplete suggestions over current nearby
                    set (already filtered to ~100 km on the backend). */}
                <div className="relative mb-2" data-testid="nearby-search-wrap">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-9 py-2.5 rounded-2xl text-xs bg-white outline-none border border-slate-200 focus:border-pink-400"
                  />
                  {nearbySearch && (
                    <button onClick={() => setNearbySearch('')}
                      data-testid="nearby-search-clear"
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
                      <X className="w-3.5 h-3.5" />
                    </button>
                  )}
                  {/* Autosuggest dropdown */}
                  {nearbySearch && nearbyFiltered.length > 0 && (
                    <div className="absolute z-30 left-0 right-0 mt-1 max-h-60 overflow-y-auto rounded-2xl bg-white border border-slate-200 shadow-2xl"
                      data-testid="nearby-search-suggestions">
                      {nearbyFiltered.slice(0, 8).map((d) => (
                        <button
                          key={d.doctor_id}
                          onClick={() => {
                            setNearbySearch('');
                            ...
                            setTimeout(() => {
                              const el = document.querySelector(`[data-testid="doctor-card-${d.doctor_id}"]`);
                          ...
                          data-testid={`nearby-suggest-${d.doctor_id}`}
                        >
                          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-[10px] font-bold shrink-0"
                            style={{ background: d.avatar_color || C.rose }}>
                            {(d.name || 'D').charAt(0)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-fredoka font-semibold truncate" style={{ color: C.ink }}>{d.name}</p>
                            <p className="text-[10px] text-slate-500 truncate">{d.specialty} · {d.distance_km ?? '—'} km</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {nearbySearch && nearbyFiltered.length === 0 && (
                    <div className="absolute z-30 left-0 right-0 mt-1 rounded-2xl bg-white border border-slate-200 shadow-2xl px-3 py-3 text-xs text-slate-400 text-center"
                      data-testid="nearby-search-no-results">
                      No nearby providers match “{nearbySearch}”.
                    </div>
                  )}
                </div>
                {nearbyFiltered.length === 0 && !nearbySearch ? (
            ...
            {doctorTab === 'subscribed' && (
              subscribed.length === 0 ? (
                ...
                subscribed.map(d => (
                  <motion.div
                    key={d.doctor_id}
                    data-testid={`subscribed-card-${d.doctor_id}`}
                    whileHover={{ y: -2 }}
                    className="rounded-2xl p-3 relative"
                    style={{ background: 'white', boxShadow: `0 10px 22px -10px ${d.avatar_color || C.sky}55`, cursor: 'pointer' }}
                    onClick={() => navigate(`/app/subscribe?tab=clinician&clinician_id=${d.doctor_id}`)}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <Check className="w-5 h-5 shrink-0" style={{ color: C.mint }} />
                    </div>
                    <div className="mt-2.5 grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        data-testid={`rate-clinician-${d.doctor_id}`}
                        onClick={(e) => { e.stopPropagation(); openRatingModal(d); }}
                        className="rounded-lg py-2 text-[10px] font-nunito font-bold flex items-center justify-center gap-1.5"
                        style={{
                          background: `linear-gradient(135deg, ${C.amber}, ${C.tangerine})`,
                          color: 'white',
                          boxShadow: `0 6px 14px -6px ${C.amber}88`,
                        }}
                      >
                      ...
                      <button
                        type="button"
                        data-testid={`unsubscribe-sub-${d.doctor_id}`}
                        onClick={(e) => { e.stopPropagation(); unsubscribeDoctor(d); }}
                        className="rounded-lg py-2 text-[10px] font-nunito font-bold flex items-center justify-center gap-1.5 border"
                        style={{
                      ...
                      >
                        <X className="w-3.5 h-3.5" /> Unsubscribe
                      </button>
                    </div>
                  </motion.div>
                ))
            ...
            {doctorTab === 'all' && (
              <>
                {/* Search bar — Enter to confirm; if no match, opens
                    "not on IFEELINCOLOR" invite popup. */}
                <div className="relative mb-3" data-testid="all-clin-search-wrap">
                  <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input
                    value={allClinSearch}
                    onChange={(e) => setAllClinSearch(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') submitAllClinSearch(); }}
                    placeholder="Search a clinician by name…"
                    data-testid="all-clin-search-input"
                    className="w-full pl-9 pr-20 py-2.5 rounded-2xl text-xs bg-white outline-none border border-slate-200 focus:border-violet-400"
                  />
                  {allClinSearch && (
                    <button onClick={() => setAllClinSearch('')}
                      data-testid="all-clin-search-clear"
                      className="absolute right-16 top-1/2 -translate-y-1/2 text-slate-400">
                      <X className="w-3.5 h-3.5" />
                    </button>
                  )}
                  <button
                    onClick={submitAllClinSearch}
                    data-testid="all-clin-search-submit"
                    className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[10px] font-nunito font-bold text-white px-3 py-1.5 rounded-xl"
                    style={{ background: `linear-gradient(135deg, ${C.violet}, ${C.bubblegum})` }}
                  >
                    Find
                  </button>
                </div>
                {allClinFiltered.length === 0 ? (
                  <div className="rounded-2xl p-6 text-center text-xs font-nunito text-slate-500"
                    style={{ background: 'rgba(255,255,255,0.7)' }} data-testid="all-clin-empty">
                    {allClinSearch
                      ? <>No clinician on IFEELINCOLOR matches “{allClinSearch}”. Tap <span className="font-bold">Find</span> to invite them.</>
                      : 'Loading clinicians…'}
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-2.5" data-testid="all-clinicians-grid">
                    {allClinFiltered.map(c => {
                    const isSubbed = !!c.is_subscribed_by_me;
                    const gotoClinicianPlans = () => {
                      navigate(`/app/subscribe?tab=clinician&clinician_id=${c.clinician_id}`);
                    };
                    const handleUnsubscribe = async (e) => {
                      e?.stopPropagation?.();
                      if (!isSubbed) return;
                      if (!window.confirm(`Cancel your subscription to ${c.name}?`)) return;
                      try {
                        await axios.delete(`${API}/patient/doctors/subscribe/${c.clinician_id}`, { withCredentials: true });
                        setPaymentMsg(`Unsubscribed from ${c.name}`);
                        await refreshSubscribed();
                        await loadAllClinicians();
                        setTimeout(() => setPaymentMsg(''), 2500);
                      } catch (err) {
                    ...
                    return (
                      <motion.div
                        key={c.clinician_id}
                        data-testid={`all-clin-card-${c.clinician_id}`}
                        whileHover={{ y: -3, scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        onClick={gotoClinicianPlans}
                        role="button"
                        tabIndex={0}
                        onKeyDown={(e) => { if (e.key === 'Enter') gotoClinicianPlans(); }}
                        className="rounded-2xl p-3 text-left relative overflow-hidden cursor-pointer"
                        style={{
                          background: 'rgba(255,255,255,0.85)',
                          backdropFilter: 'blur(14px)',
                          border: isSubbed
                            ? `2px solid ${C.mint}88`
                            : `1px solid ${C.violet}33`,
                          boxShadow: `0 12px 24px -10px ${C.violet}55`,
                        }}
                      >
                        ...
                        <motion.div
                          ...
                          style={{ background: `radial-gradient(circle, ${C.tangerine}, transparent 70%)` }}
                        />
                        <div className="relative flex flex-col items-center">
                          <div className="relative">
                            <div
                              ...
                              style={{
                                ...
                                boxShadow: `0 12px 22px -10px ${C.rose}66`,
                              }}
                            >
                              {c.profile_photo
                                ? <img src={c.profile_photo} alt="" className="w-full h-full object-cover" />
                                : (c.name || 'D').charAt(0)}
                            </div>
                            {isSubbed ? (
                              <span
                                ...
                                style={{
                                  ...
                                  boxShadow: `0 4px 10px ${C.mint}88`,
                                }}
                                data-testid={`all-clin-subbed-badge-${c.clinician_id}`}
                              >
                              ...
                              <span
                                ...
                                style={{
                                  ...
                                  boxShadow: `0 4px 10px ${C.violet}88`,
                                }}
                              >
                          ...
                          <p className="font-fredoka font-semibold text-xs mt-2 text-center truncate w-full" style={{ color: C.ink }}>{c.name}</p>
                          <p className="text-[10px] font-nunito text-center truncate w-full" style={{ color: '#7B6A9C' }}>
                            {c.years_experience != null ? `${c.years_experience} yrs exp` : c.subscription_tier}
                          </p>
                        </div>
                        {/* Subscribe / Unsubscribe pair (one always disabled based on state) */}
                        <div className="mt-2.5 grid grid-cols-2 gap-1.5">
                          <button
                            type="button"
                            data-testid={`all-clin-subscribe-${c.clinician_id}`}
                            disabled={isSubbed}
                            onClick={(e) => { e.stopPropagation(); if (!isSubbed) gotoClinicianPlans(); }}
                            className="text-[9px] font-nunito font-bold py-1.5 rounded-lg transition-all"
                            style={{
                              background: isSubbed
                                ? '#E8E5F2'
                                : `linear-gradient(135deg, ${C.rose}, ${C.tangerine})`,
                              color: isSubbed ? '#A39AB8' : 'white',
                              boxShadow: isSubbed ? 'none' : `0 6px 14px -6px ${C.rose}88`,
                              cursor: isSubbed ? 'not-allowed' : 'pointer',
                              opacity: isSubbed ? 0.6 : 1,
                            }}
                          >
                            Subscribe
                          </button>
                          <button
                            type="button"
                            data-testid={`all-clin-unsubscribe-${c.clinician_id}`}
                            disabled={!isSubbed}
                            onClick={handleUnsubscribe}
                            className="text-[9px] font-nunito font-bold py-1.5 rounded-lg transition-all border"
                            style={{
                              background: isSubbed ? 'rgba(239,68,68,0.10)' : '#F4F2F8',
                              color: isSubbed ? '#DC2626' : '#A39AB8',
                              borderColor: isSubbed ? 'rgba(239,68,68,0.35)' : 'transparent',
                              cursor: isSubbed ? 'pointer' : 'not-allowed',
                              opacity: isSubbed ? 1 : 0.6,
                            }}
                          >
                            Unsubscribe
                          </button>
      ...
      {/* ─── Rating + feedback modal (My Subscribed tab) ─── */}
      <AnimatePresence>
        {ratingModalDoc && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-sm rounded-3xl p-6 bg-white relative"
              style={{ boxShadow: `0 40px 80px -20px ${C.amber}99`, border: `1px solid ${C.amber}33` }}
            >
              <button onClick={() => setRatingModalDoc(null)}
              ...
              <div className="text-center">
                <div className="w-14 h-14 mx-auto rounded-2xl flex items-center justify-center text-white font-bold text-xl mb-2"
                  style={{ background: ratingModalDoc.avatar_color || C.rose }}>
                  {(ratingModalDoc.name || 'D').charAt(0)}
                </div>
                <p className="text-[10px] font-nunito font-bold uppercase tracking-widest" style={{ color: C.amber }}>
                  Rate &amp; review
                </p>
                <h3 className="font-fredoka font-semibold text-lg mt-0.5" style={{ color: C.ink }}>
                  {ratingModalDoc.name}
                </h3>
                <p className="text-[11px] font-nunito" style={{ color: '#7B6A9C' }}>{ratingModalDoc.specialty}</p>
              </div>
              <div className="mt-4 flex items-center justify-center gap-2" data-testid="rating-stars">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    key={n}
                    type="button"
                    ...
                    style={{ background: n <= ratingValue ? `${C.amber}22` : '#F4F2F8' }}
                  >
                    <Star className={`w-6 h-6 ${n <= ratingValue ? 'fill-current' : ''}`}
                      style={{ color: n <= ratingValue ? C.amber : '#CBC5DC' }} />
                  </button>
                ))}
              </div>
              <textarea
                ...
                placeholder={`Share how ${(ratingModalDoc.name || '').split(' ')[0] || 'they'} helped you…`}
                rows={4}
                maxLength={2000}
                className="mt-4 w-full rounded-2xl border border-slate-200 p-3 text-xs font-nunito outline-none focus:border-amber-400 resize-none"
              />
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button
                  ...
                  onClick={() => setRatingModalDoc(null)}
                  className="rounded-xl py-2.5 text-xs font-nunito font-bold"
                  style={{ background: '#F4F2F8', color: '#7B6A9C' }}
                >
                  Cancel
                </button>
                <button
                  ...
                  className="rounded-xl py-2.5 text-xs font-nunito font-bold text-white disabled:opacity-50"
                  style={{
                    background: `linear-gradient(135deg, ${C.amber}, ${C.tangerine})`,
                    boxShadow: `0 10px 22px -8px ${C.amber}88`,
                  }}
                >
                  Save rating
                </button>
      ...
      {/* ─── Not-on-IFEELINCOLOR invite popup (All Clinicians search miss) ─── */}
      <AnimatePresence>
        {notFoundModal && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm"
            data-testid="not-found-modal"
          ...
          >
            <motion.div
              ...
              transition={{ type: 'spring', stiffness: 230, damping: 22 }}
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-sm rounded-3xl p-6 bg-white relative text-center"
              style={{ boxShadow: `0 40px 80px -20px ${C.violet}99`, border: `1px solid ${C.violet}33` }}
            >
              <button onClick={() => setNotFoundModal(null)}
                data-testid="not-found-close"
                className="absolute top-3 right-3 w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center">
                <X className="w-3.5 h-3.5 text-slate-500" />
              </button>
              <motion.div
                animate={{ rotate: [0, 6, -6, 0] }} transition={{ duration: 4, repeat: Infinity }}
                className="w-16 h-16 mx-auto rounded-3xl flex items-center justify-center text-white text-2xl font-bold"
                style={{
                  background: `linear-gradient(135deg, ${C.violet}, ${C.bubblegum})`,
                  boxShadow: `0 16px 30px -8px ${C.violet}88`,
                }}>
                <Sparkles className="w-8 h-8" />
              </motion.div>
              <h3 className="font-fredoka font-semibold text-lg mt-3" style={{ color: C.ink }}>
                “{notFoundModal.name}” isn't on IFEELINCOLOR yet
              </h3>
              <p className="text-[12px] font-nunito mt-2" style={{ color: '#6B5784' }}>
                Tell them about our platform — once they join, you can subscribe to their care and
                continue your journey here together.
              </p>
              <button
                type="button"
                data-testid="not-found-share"
                onClick={() => { shareInvite(notFoundModal.name); setNotFoundModal(null); }}
                className="mt-5 w-full rounded-2xl py-3 text-sm font-nunito font-bold text-white flex items-center justify-center gap-2"
                style={{
                  background: `linear-gradient(135deg, ${C.violet}, ${C.bubblegum})`,
                  boxShadow: `0 16px 30px -8px ${C.violet}aa`,
                }}
              >
                <Share2 className="w-4 h-4" /> Share invite to {notFoundModal.name}
              </button>
              <button
                type="button"
                onClick={() => setNotFoundModal(null)}
                className="mt-2 w-full text-[11px] font-nunito font-bold"
                style={{ color: '#8F84A8' }}
              >
                Maybe later
              </button>
      ...
      <AnimatePresence>
        {maskedModal && (
          <motion.div
            ...
            onClick={() => setMaskedModal(null)}
            data-testid="masked-clin-modal"
          >
            <motion.div
              initial={{ y: 40, opacity: 0, scale: 0.95 }}
              animate={{ y: 0, opacity: 1, scale: 1 }}
              ...
              style={{
                background: 'linear-gradient(180deg, rgba(255,255,255,0.96), rgba(255,247,255,0.96))',
                boxShadow: `0 40px 80px -20px ${C.lilac}99`,
            ...
            >
              <div className="p-6 text-center">
                ...
                >
                  <div
                    className="absolute inset-0 rounded-3xl flex items-center justify-center text-white text-2xl font-bold overflow-hidden"
                    style={{
                      background: `linear-gradient(135deg, ${C.rose}, ${C.tangerine}, ${C.lilac})`,
                      boxShadow: `0 18px 32px -10px ${C.rose}88, inset 0 4px 10px rgba(255,255,255,0.45)`,
                    }}
                  >
                    {maskedModal.profile_photo
                      ? <img src={maskedModal.profile_photo} alt="" className="w-full h-full object-cover" />
                      : (maskedModal.name || 'D').charAt(0)}
                  </div>
                </motion.div>
                <p className="text-xs font-nunito font-bold uppercase tracking-[0.2em] mt-4" style={{ color: C.rose }}>
                  Subscribe to unlock
                </p>
                <h2 className="text-xl font-fredoka font-bold mt-1" style={{ color: C.ink }}>
                  {maskedModal.name}
                </h2>
                <p className="text-xs font-nunito mt-1" style={{ color: '#7B6A9C' }}>
                  {maskedModal.years_experience != null ? `${maskedModal.years_experience} yrs experience · ` : ''}
                  {maskedModal.specialty}
                </p>
                {/* Masked fields */}
                <div className="mt-4 space-y-1.5 text-left">
                  {['Bio', 'Phone', 'Address', 'Email'].map((f) => (
                    <div key={f} className="flex items-center gap-2 px-3 py-2 rounded-lg"
                      style={{ background: `${C.lilac}11`, border: `1px dashed ${C.lilac}55` }}>
                      <LockIcon className="w-3 h-3" style={{ color: C.lilac }} />
                      <span className="text-[11px] font-nunito font-bold" style={{ color: C.lilac }}>{f}:</span>
                      <span className="text-[11px] font-nunito tracking-widest" style={{ color: '#9785B5' }}>••••••••••</span>
                    </div>
                  ))}
                </div>
                <p className="text-[10px] font-nunito mt-3" style={{ color: '#7B6A9C' }}>
                  Subscribe to view full profile, contact details, and book sessions.
                </p>
                <div className="flex gap-2 mt-5">
                  <button
                    onClick={() => setMaskedModal(null)}
                    data-testid="masked-modal-close"
                    className="flex-1 rounded-xl py-2.5 text-xs font-nunito font-bold"
                    style={{ background: 'rgba(123,106,156,0.08)', color: '#7B6A9C' }}
                  >
                    Later
                  </button>
                  <button
                    onClick={() => {
                      const cid = maskedModal.clinician_id;
                      setMaskedModal(null);
                      navigate(`/app/subscribe?tab=clinician&clinician_id=${cid}`);
                    }}
                    data-testid="masked-modal-subscribe"
                    className="flex-1 rounded-xl py-2.5 text-xs font-nunito font-bold text-white"
                    style={{
                      background: `linear-gradient(135deg, ${C.rose}, ${C.tangerine})`,
                      boxShadow: `0 10px 22px -8px ${C.rose}88`,
                    }}
                  >
                    Subscribe →
                  </button>
