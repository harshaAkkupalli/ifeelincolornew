...
import {
  ...
  Calendar, BadgeCheck, Clock, ExternalLink, Loader2, Search, X as XIcon,
} from 'lucide-react';
...
export default function PatientSubscribe() {
  ...
  // Payment-outcome modal (success / error / info). Replaces toasts for
  // Razorpay results so the message is always visible above the dimmed
  // Razorpay backdrop instead of getting lost under it.
  const [paymentResult, setPaymentResult] = useState(null); // {kind, title, message, onPrimary?, primaryLabel?}
  const pollRef = useRef(null);
  const { configured: rzpConfigured, launchCheckout } = useRazorpayCheckout();

  // Clinician-search state — Clinician tab uses an autocomplete to pick which
  // clinician's plans to show.
  const [clinSearch, setClinSearch] = useState('');
  const [clinResults, setClinResults] = useState([]);
  const [selectedClin, setSelectedClin] = useState(null); // { clinician_id, name, ... }
  const [clinPlans, setClinPlans] = useState([]);
  const [otherClinicians, setOtherClinicians] = useState([]); // shown below the plan list
  const searchTimer = useRef(null);

  // Deep link support: /app/subscribe?tab=clinician&clinician_id=...
  useEffect(() => {
    const deepTab = searchParams.get('tab');
    const deepClinId = searchParams.get('clinician_id');
    if (deepTab && ['portal', 'clinician', 'organization'].includes(deepTab)) {
      setTab(deepTab);
    }
    if (deepClinId) {
      setTab('clinician');
      // Resolve the clinician card (for header + sub state) AND eagerly fetch
      // their plans so the patient sees admin-published plans immediately —
      // even if /clinicians/discover hasn't returned yet.
      (async () => {
        try {
          const r = await axios.get(`${API}/clinicians/discover`, { withCredentials: true });
          const found = (r.data.clinicians || []).find(c => c.clinician_id === deepClinId);
          if (found) {
            setSelectedClin(found);
          } else {
            // Discover didn't yield this clinician — synthesize a minimal
            // record so the plan fetch can still happen.
            setSelectedClin({ clinician_id: deepClinId, name: 'Clinician', specialty: '' });
          }
        } catch {
          setSelectedClin({ clinician_id: deepClinId, name: 'Clinician', specialty: '' });
        }
  ...
  // Autocomplete query → /api/clinicians/discover?q=
  useEffect(() => {
    if (tab !== 'clinician') return;
    if (searchTimer.current) clearTimeout(searchTimer.current);
    searchTimer.current = setTimeout(async () => {
      try {
        const r = await axios.get(`${API}/clinicians/discover`,
          { params: clinSearch ? { q: clinSearch } : {}, withCredentials: true });
        setClinResults((r.data.clinicians || []).slice(0, 12));
      } catch { /* ignore */ }
    }, 250);
    return () => searchTimer.current && clearTimeout(searchTimer.current);
  }, [clinSearch, tab]);

  // Load plans for selected clinician
  useEffect(() => {
    if (!selectedClin) { setClinPlans([]); return; }
    (async () => {
      try {
        const r = await axios.get(`${API}/plans/patient-clinician`,
          { params: { clinician_id: selectedClin.clinician_id }, withCredentials: true });
        setClinPlans(r.data.plans || []);
      } catch { setClinPlans([]); }
    })();
  }, [selectedClin]);

  // Load "other clinicians" strip — shown below the selected clinician's plans
  // so the patient can quickly jump between care providers.
  useEffect(() => {
    if (tab !== 'clinician' || !selectedClin) { setOtherClinicians([]); return; }
    (async () => {
      try {
        const r = await axios.get(`${API}/clinicians/discover`, { withCredentials: true });
        const all = r.data.clinicians || [];
        setOtherClinicians(all.filter(c => c.clinician_id !== selectedClin.clinician_id).slice(0, 20));
      } catch { setOtherClinicians([]); }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, selectedClin?.clinician_id]);
  ...
  const list = tab === 'clinician' && selectedClin ? clinPlans : (plans[tab] || []);
  const meta = TAB_META[tab];
  const isPortalActive = !!(activeSub && activeSub.active);

  // Patient's active subscription for the currently-selected clinician (if any).
  // We compare on doctor_id which is what /patient/doctors/subscribed returns.
  const myDocSub = (tab === 'clinician' && selectedClin)
    ? doctorSubs.find(d => d.doctor_id === selectedClin.clinician_id
                        || d.clinician_id === selectedClin.clinician_id)
    : null;
  ...
  const subscribeFlow = async (plan) => {
    ...
    try {
      ...
      if (!rzpConfigured) {
        setPaymentResult({ presetKey: 'configMissing' });
        setSubmitting(false);
        return;
      }
      ...
      await launchCheckout({
        ...
        onSuccess: async (data) => {
          ...
          setPaymentResult({
            presetKey: 'success',
            // Personalised over-rides while keeping the warm patient tone
            title: `Yay! ${data.plan_name || plan.name} is unlocked ✨`,
            onPrimary: () => { setPaymentResult(null); navigate('/app/assessment'); },
          });
        },
        onFailure: (err) => {
          setSubmitting(false);
          if (err?.dismissed) {
            setPaymentResult({
              presetKey: 'dismissed',
              onPrimary: () => { setPaymentResult(null); subscribeFlow(plan); },
            });
          } else {
            setPaymentResult({
              presetKey: 'failure',
              message: err?.message
                ? `Don't worry, no money was deducted. (${err.message}) Take a breath, and whenever you're ready we can try again together.`
                : undefined,
              onPrimary: () => { setPaymentResult(null); subscribeFlow(plan); },
            });
          }
  ...
  return (
    <div className="px-5 pt-5 pb-10 relative">
      <PaymentResultDialog
        open={!!paymentResult}
        role="patient"
        presetKey={paymentResult?.presetKey}
        kind={paymentResult?.kind}
        title={paymentResult?.title}
        message={paymentResult?.message}
        onClose={() => setPaymentResult(null)}
        onPrimary={paymentResult?.onPrimary}
        primaryLabel={paymentResult?.primaryLabel}
      />
      <AnimatePresence>
        {toast && (
      ...
      {/* Plan list */}
      <div className="mt-5">
        {/* Clinician tab — search & pick a clinician first */}
        {tab === 'clinician' && (
          <div className="mb-5">
            {!selectedClin ? (
              <>
                <p className="text-xs font-nunito font-bold uppercase tracking-widest mb-2" style={{ color: TAB_META.clinician.color }}>
                  Search a clinician
                </p>
                <div className="relative">
                  ...
                  <input
                    value={clinSearch}
                    onChange={(e) => setClinSearch(e.target.value)}
                    placeholder="Type a clinician name…"
                    data-testid="clin-search-input"
                    className="w-full pl-9 pr-9 py-3 rounded-2xl text-sm bg-white outline-none border border-slate-200 focus:border-teal-400"
                  />
                  {clinSearch && (
                    <button onClick={() => setClinSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
                      <XIcon className="w-4 h-4" />
                    </button>
                  )}
                </div>
                {clinResults.length > 0 && (
                  <div className="mt-3 space-y-2" data-testid="clin-search-results">
                    {clinResults.map((c) => (
                      <motion.button
                        key={c.clinician_id}
                        whileHover={{ x: 3 }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => setSelectedClin(c)}
                        data-testid={`clin-pick-${c.clinician_id}`}
                        className="w-full flex items-center gap-3 p-3 rounded-2xl bg-white text-left transition border"
                        style={{ borderColor: '#E2E8F0', boxShadow: '0 6px 18px -10px rgba(34,211,197,0.18)' }}
                      >
                        <div
                          className="w-11 h-11 rounded-xl flex items-center justify-center text-white font-bold overflow-hidden shrink-0"
                          style={{ background: 'linear-gradient(135deg, #22D3C5, #0E7490)' }}
                        >
                          {c.profile_photo
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-fredoka font-semibold text-sm truncate" style={{ color: '#2A1A4A' }}>{c.name}</p>
                          <p className="text-[11px] font-nunito text-slate-500 truncate">
                            {c.years_experience != null ? `${c.years_experience} yrs · ` : ''}{c.specialty}
                          </p>
                        </div>
                        {c.is_subscribed_by_me ? (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-green-50 text-green-700">Subscribed</span>
                        ) : (
                ...
                {clinResults.length === 0 && clinSearch.length > 0 && (
                  <p className="mt-3 text-xs text-slate-400 italic">No matching clinicians.</p>
                )}
              </>
            ) : (
              <div className="rounded-2xl p-3 bg-white border-2 flex items-center gap-3"
                style={{ borderColor: `${TAB_META.clinician.color}55` }}
                data-testid="clin-selected-card">
                <div
                  className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold overflow-hidden shrink-0"
                  style={{ background: 'linear-gradient(135deg, #22D3C5, #0E7490)' }}
                >
                  {selectedClin.profile_photo
                    ? <img src={selectedClin.profile_photo} alt="" className="w-full h-full object-cover" />
                    : (selectedClin.name || 'D').charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] uppercase tracking-widest font-bold" style={{ color: TAB_META.clinician.color }}>Showing plans for</p>
                  <p className="font-fredoka font-semibold truncate" style={{ color: '#2A1A4A' }}>{selectedClin.name}</p>
                  <p className="text-[11px] font-nunito text-slate-500 truncate">
                    {selectedClin.years_experience != null ? `${selectedClin.years_experience} yrs · ` : ''}{selectedClin.specialty}
                  </p>
                </div>
                <button
                  data-testid="clin-clear-selected"
                  onClick={() => { setSelectedClin(null); setClinSearch(''); }}
                  className="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-100 text-slate-500"
                >
                  <XIcon className="w-4 h-4" />
                </button>
              </div>
            ...
            {list.map((p) => {
              ...
              return (
                <motion.div key={p.plan_id || p._id}
                  ...
                  {/* Per-card Subscribe / Unsubscribe pair.
                      • If THIS plan is the patient's currently-active plan →
                        Subscribe is disabled, Unsubscribe is live.
                      • Otherwise → Subscribe is live, Unsubscribe is disabled.
                      When the patient already has a DIFFERENT plan for this
                      clinician/portal, tapping Subscribe asks them to confirm
                      a plan-switch first. */}
                  {(() => {
                    const pid = p.plan_id || p._id;
                    // Per-plan precision: only the plan the patient actually
                    // subscribed to gets isCurrent=true.
                    const isCurrent = tab === 'portal'
                      ? !!(activeSub?.subscription?.plan_id === pid && activeSub?.active)
                      : tab === 'clinician'
                        ? !!(myDocSub && myDocSub.plan_id === pid)
                        : false;
                    // Patient has an active sub on this tab but to a different plan
                    const otherSubName = tab === 'portal'
                      ? (isPortalActive && activeSub?.subscription?.plan_id !== pid
                          ? (activeSub?.subscription?.plan_name || 'your current plan')
                          : null)
                      : tab === 'clinician'
                        ? (myDocSub && myDocSub.plan_id !== pid
                            ? (myDocSub.plan_name || 'your current plan')
                            : null)
                        : null;
                    const subBg = isFree
                    ...
                    const doClinicianSubscribe = async () => {
                      const price = parseFloat(p.price_usd ?? p.price ?? 0);
                      try {
                        // If patient already has a sub on a DIFFERENT plan for
                        // this clinician, switch by unsubscribing first.
                        if (otherSubName) {
                          await axios.delete(
                            `${API}/patient/doctors/subscribe/${selectedClin.clinician_id}`,
                            { withCredentials: true }
                          );
                        }
                        // ── FREE / TRIAL plan → mock activation (no Razorpay) ──
                        if (price <= 0) {
                          await axios.post(
                            `${API}/patient/doctors/subscribe-paid`,
                            { doctor_id: selectedClin.clinician_id, plan_id: pid },
                            { withCredentials: true }
                          );
                          showToast(otherSubName
                            ? `Plan changed to ${p.name || p.plan_name} for ${selectedClin.name}!`
                            : `Trial activated for ${selectedClin.name}!`);
                          await load();
                          setSelectedClin({ ...selectedClin, is_subscribed_by_me: true });
                          return;
                        }
                        // ── PAID plan → must go through Razorpay ──
                        if (!rzpConfigured) {
                          setPaymentResult({ presetKey: 'configMissing' });
                          return;
                        }
                        await launchCheckout({
                          kind: 'patient_doctor',
                          plan_id: pid,
                          doctor_id: selectedClin.clinician_id,
                          onSuccess: async () => {
                            await load();
                            setSelectedClin({ ...selectedClin, is_subscribed_by_me: true });
                            setPaymentResult({
                              presetKey: 'success',
                              title: otherSubName
                                ? `Plan changed to ${p.name || p.plan_name} ✨`
                                : `You're connected with ${selectedClin.name.split(' ')[0]} 💜`,
                              message: otherSubName
                                ? `Your care is now with ${selectedClin.name}. We've notified them so they can welcome you.`
                                : `${selectedClin.name} now has your back. They'll see your check-ins as soon as you start.`,
                              primaryLabel: 'Lovely',
                              onPrimary: () => setPaymentResult(null),
                            });
                          },
                          onFailure: (err) => {
                            if (err?.dismissed) {
                              setPaymentResult({ presetKey: 'dismissed' });
                            } else {
                              setPaymentResult({
                                presetKey: 'failure',
                                message: err?.message
                                  ? `Don't worry, no money was deducted. (${err.message}) Take a breath, and we'll try again together.`
                                  : undefined,
                              });
                            }
                          },
                        ...
                        showToast(err?.response?.data?.detail || 'Could not subscribe');
                      }
                    };
                    ...
                    return (
                      <div className="mt-4 grid grid-cols-2 gap-2">
                        ...
                        <motion.button
                          ...
                          disabled={isCurrent}
                          data-testid={`subscribe-plan-btn-${pid}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            if (isCurrent) return;
                            if (tab === 'clinician') {
                              if (selectedClin) {
                                // Plan-switch confirm — patient is already on a
                                // different plan for this clinician.
                                if (otherSubName) {
                                  if (!window.confirm(
                                    `You're currently subscribed to "${otherSubName}" with ${selectedClin.name}. Change to "${p.name || p.plan_name}"?`
                                  )) return;
                                }
                                doClinicianSubscribe();
                                return;
                              }
                            ...
                            if (otherSubName) {
                              if (!window.confirm(
                                `You're currently subscribed to "${otherSubName}". Change to "${p.name || p.plan_name}"?`
                              )) return;
                            }
                            setSelectedPlan(pid);
                          }}
                          className="rounded-2xl py-2.5 text-xs font-nunito font-bold flex items-center justify-center gap-1.5 transition"
                          style={{
                            background: isCurrent ? '#E8E5F2' : subBg,
                            color: isCurrent ? '#A39AB8' : 'white',
                            boxShadow: isCurrent ? 'none' : `0 12px 24px -10px ${isFree ? '#22D67E' : meta.color}88`,
                            cursor: isCurrent ? 'not-allowed' : 'pointer',
                            opacity: isCurrent ? 0.65 : 1,
                          }}
                        >
                          {isFree ? (
                            <><Sparkles className="w-3.5 h-3.5" /> Trial</>
                          ) : otherSubName ? (
                            <><Shield className="w-3.5 h-3.5" /> Change plan</>
                          ) : (
                            <><Shield className="w-3.5 h-3.5" /> Subscribe</>
                          )}
                        </motion.button>
                        ...
                        <motion.button
                          whileTap={isCurrent ? { scale: 0.97 } : {}}
                          disabled={!isCurrent}
                          data-testid={`unsubscribe-plan-btn-${pid}`}
                          onClick={async (e) => {
                            e.stopPropagation();
                            if (!isCurrent) return;
                            if (!window.confirm('Cancel this subscription? You will keep access until the end of your billing period.')) return;
                            try {
                              if (tab === 'clinician' && selectedClin) {
                                await axios.delete(
                                  `${API}/patient/doctors/subscribe/${selectedClin.clinician_id}`,
                                  { withCredentials: true }
                                );
                                showToast(`Unsubscribed from ${selectedClin.name}`);
                                // Flip the local subscribed flag so Subscribe re-enables.
                                setSelectedClin({ ...selectedClin, is_subscribed_by_me: false });
                              } else {
                                await axios.post(`${API}/me/subscription/cancel`, {}, { withCredentials: true });
                                showToast('Subscription cancelled');
                              }
                              await load();
                            } catch (err) {
                              showToast(err?.response?.data?.detail || 'Could not unsubscribe');
                            }
                          }}
                          className="rounded-2xl py-2.5 text-xs font-nunito font-bold flex items-center justify-center gap-1.5 transition border"
                          style={{
                            background: isCurrent ? 'rgba(239,68,68,0.08)' : '#F4F2F8',
                            color: isCurrent ? '#DC2626' : '#A39AB8',
                            borderColor: isCurrent ? 'rgba(239,68,68,0.35)' : 'transparent',
                ...
                <motion.div
                  ...
                  <Button
                    ...
                    {submitting ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin inline" /> Working…</>)
                     ...
                     : (<><Shield className="w-4 h-4 mr-1.5 inline -mt-0.5" /> Pay ${amount} with Razorpay</>)}
                  </Button>
        ...
        {/* Other clinicians strip — selected clinician is on top (plan list
            above), others appear here next so the patient can quickly jump
            between providers without going back to /app/home. */}
        {tab === 'clinician' && selectedClin && otherClinicians.length > 0 && (
          <div className="mt-6" data-testid="other-clinicians-strip">
            <p className="text-[10px] uppercase tracking-widest font-bold mb-2" style={{ color: TAB_META.clinician.color }}>
              Other clinicians ({otherClinicians.length})
            </p>
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 -mx-1 px-1 snap-x">
                ...
                <motion.button
                  ...
                  onClick={() => {
                    ...
                    try {
                      ...
                      u.searchParams.set('tab', 'clinician');
                      u.searchParams.set('clinician_id', c.clinician_id);
                      window.history.replaceState({}, '', u.toString());
                    } catch { /* noop */ }
                  }}
                  data-testid={`other-clin-card-${c.clinician_id}`}
                  className="shrink-0 snap-start w-28 rounded-2xl p-2.5 bg-white relative overflow-hidden text-center border"
                  style={{
                    borderColor: c.is_subscribed_by_me ? `${TAB_META.clinician.color}88` : '#E2E8F0',
                    boxShadow: '0 8px 18px -10px rgba(34,211,197,0.25)',
                  }}
                >
                  <div className="relative w-12 h-12 mx-auto rounded-xl flex items-center justify-center text-white font-bold overflow-hidden"
                    style={{ background: 'linear-gradient(135deg, #22D3C5, #0E7490)' }}>
                    {c.profile_photo
                      ? <img src={c.profile_photo} alt="" className="w-full h-full object-cover" />
                      : (c.name || 'D').charAt(0)}
                    {c.is_subscribed_by_me && (
                      <span className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center"
                        style={{ background: '#22D67E', boxShadow: '0 4px 8px rgba(34,214,126,0.5)' }}>
                        <Check className="w-3 h-3 text-white" />
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] font-fredoka font-semibold mt-1.5 truncate" style={{ color: '#2A1A4A' }}>{c.name}</p>
                  <p className="text-[9px] font-nunito text-slate-500 truncate">
                    {c.years_experience != null ? `${c.years_experience} yrs` : 'Specialist'}
                  </p>
                </motion.button>
              ))}
            </div>
