...
export default function PatientAssessmentRun() {
  ...
  if (pendingMsg) {
    if (isFinalCat) {
      ...
      // PORTAL the CheckInFlow to document.body so its fixed positioning
      // escapes the swipeable parent's `transform`+`overflow:hidden`
      // containing block (which was clipping the flow to a blank area).
      return createPortal(
        <CheckInFlow
  ...
  // PORTAL so it escapes the swipeable container's clipping box.
  if (runCheckin) {
    return createPortal(
      <CheckInFlow
