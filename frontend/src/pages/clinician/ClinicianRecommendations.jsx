/**
 * Clinician Recommendations Library — UI/UX MATCHES Admin's RecommendationsPage 1:1.
 *
 * The only differences vs. Admin:
 *   • Uses clinician-scoped endpoints:
 *     - GET    /api/clinician/recommendation-library      (list — own + admin library)
 *     - POST   /api/clinician/custom-recommendations      (create)
 *     - PUT    /api/clinician/custom-recommendations/{id} (edit own only)
 *     - DELETE /api/clinician/custom-recommendations/{id} (delete own only)
 *     - GET    /api/clinician/dashboard-stats             (patient roster — STRICT subscribed only)
 *     - POST   /api/clinician/recommend                   (send to a subscribed patient)
 *   • Send-to-Patient picker only shows STRICTLY-subscribed patients (USP).
 */
...
const EMPTY_FORM = {
  title: '', description: '', target_conditions: '', category: 'general',
  content_type: 'text', media_url: '', image_url: '', steps: '',
  severity_filter: [], duration_minutes: '', trigger_colors: [], body_md: '',
};
...
export default function ClinicianRecommendations() {
  ...
  const load = async () => {
    try {
      const r = await ax.get(`${API}/clinician/recommendation-library`);
      // Clinician library returns: { recommendations: [{ rec_id|recommendation_id, title, ... }] }
      const list = (r.data.recommendations || []).map((it) => ({
        recommendation_id: it.rec_id || it.recommendation_id,
        rec_id: it.rec_id || it.recommendation_id,
        ...it,
  ...
  const openEdit = (r) => {
    ...
    setForm({
      ...EMPTY_FORM, ...r,
      target_conditions: (r.target_conditions || []).join(', '),
      steps: (r.steps || []).join('\n'),
      trigger_colors: r.trigger_colors || [],
      body_md: r.body_md || '',
    });
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (editItem) {
      await ax.put(`${API}/clinician/custom-recommendations/${editItem.rec_id || editItem.recommendation_id}`, payload);
    } else {
      await ax.post(`${API}/clinician/custom-recommendations`, payload);
    }
    setShowForm(false);
    load();
  };
  ...
  const handleDelete = async () => {
    await ax.delete(`${API}/clinician/custom-recommendations/${deleteId}`);
    setDeleteId(null);
    load();
  };

  const toggleColor = (key) => setForm((f) => ({
    ...f,
    trigger_colors: f.trigger_colors?.includes(key)
      ? f.trigger_colors.filter((c) => c !== key)
      : [...(f.trigger_colors || []), key],
  }));
    ...
    <div className="p-6 max-w-7xl mx-auto bg-slate-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Library of recommendations you can send to your subscribed patients.</p>
        </div>
        <Button onClick={openCreate} data-testid="add-recommendation-btn"
          className="text-xs rounded-lg bg-emerald-700 text-white border-0 hover:bg-emerald-800">
          <Plus className="w-3.5 h-3.5 mr-1" /> Add Recommendation
        </Button>
      </div>

      <div className="grid gap-4">
        {items.map((r) => (
          <div key={r.recommendation_id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-lg bg-emerald-50 flex items-center justify-center mt-0.5">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 capitalize">{r.category || 'general'}</span>
                    {(r.target_conditions || []).map((c) => (
                      <span key={c} className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">{c}</span>
                    ))}
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${r.status === 'active' ? 'bg-green-50 text-green-600' : 'bg-slate-100 text-slate-400'}`}>{r.status || 'active'}</span>
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="sm" onClick={() => setSendModal({ rec: r })}
                  className="text-pink-500 p-1" data-testid={`send-rec-${r.recommendation_id}`} title="Send to a subscribed patient">
                  <Send className="w-3.5 h-3.5" />
                </Button>
                {r.owner === 'clinician' && (
                  <>
                    <Button variant="ghost" size="sm" onClick={() => openEdit(r)} className="text-blue-500 p-1">
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => setDeleteId(r.rec_id || r.recommendation_id)} className="text-red-500 p-1">
        ...
        {!items.length && (
          <div className="bg-white rounded-xl border p-12 text-center text-sm text-slate-400">
            No recommendations yet. Create one based on your patient's assessment patterns.
          </div>
        )}
      </div>

      {/* Editor dialog (matches admin) */}
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>{editItem ? 'Edit' : 'Create'} Recommendation</DialogTitle></DialogHeader>
          <div className="space-y-3 mt-2 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <Select value={form.content_type} onValueChange={(v) => setForm({ ...form, content_type: v })}>
                  <SelectTrigger className="rounded-lg" data-testid="rec-content-type"><SelectValue /></SelectTrigger>
                  <SelectContent>{['text','video','image','steps','link'].map((c) => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
                ...
                {['critical','high','moderate','low'].map((s) => {
                  ...
                  return (
                    <button key={s} type="button"
                      onClick={() => setForm({ ...form, severity_filter: on ? form.severity_filter.filter((x) => x !== s) : [...(form.severity_filter || []), s] })}
                      className="text-[10px] font-bold px-2 py-1 rounded-full border capitalize"
                      style={{ background: on ? '#1F6F54' : 'white', color: on ? 'white' : '#1F6F54', borderColor: '#1F6F54' }}>
                      {s}
                    </button>
                  );
                })}
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={handleSave} data-testid="save-rec-btn" className="rounded-lg text-xs bg-emerald-700 text-white border-0 hover:bg-emerald-800">Save</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>Delete Recommendation?</DialogTitle></DialogHeader>
...
/**
 * Sends a recommendation to ONE of the clinician's SUBSCRIBED patients.
 * USP: list is sourced from /api/clinician/dashboard-stats (strict subscribed-only).
 */
function SendToSubscribedPatientModal({ rec, onClose, onSent }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const r = await ax.get(`${API}/clinician/dashboard-stats`);
        if (!cancelled) setPatients(r.data.patients || []);
      } catch {
        try {
          const r2 = await ax.get(`${API}/clinician/patients`);
  ...
  const send = async (p) => {
    ...
    try {
      await ax.post(`${API}/clinician/recommend`, {
        patient_id: p.user_id,
        recommendation_id: rec.rec_id || rec.recommendation_id,
      });
      onSent();
    } catch (e) {
      ...
      <DialogContent className="sm:max-w-lg" data-testid="send-to-patient-modal">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Send className="w-4 h-4 text-pink-500" /> Send to a subscribed patient
          </DialogTitle>
          <p className="text-[11px] text-slate-500 -mt-1">
            Sending <span className="font-bold text-slate-800">{rec.title}</span>
            {rec.severity_filter?.length > 0 && (<> · matches severity: {rec.severity_filter.join(', ')}</>)}
          </p>
        </DialogHeader>
        <div className="space-y-2 mt-2">
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input data-testid="send-search" value={query} onChange={(e) => setQuery(e.target.value)}
                placeholder="Search by name or email..." className="rounded-lg pl-7 text-sm h-9" />
            </div>
            <Select value={filterSev} onValueChange={setFilterSev}>
              <SelectTrigger className="w-32 h-9 rounded-lg text-xs" data-testid="send-severity"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="max-h-80 overflow-y-auto space-y-1.5 pr-1">
            ...
            {!loading && filtered.length === 0 && (
              <p className="text-xs text-slate-400 text-center py-6">
                <Users className="w-4 h-4 mx-auto mb-1" /> No subscribed patients match this filter.
              </p>
            )}
            {filtered.map((p) => {
              const sev = (p.latest_severity || p.severity || '').toLowerCase();
              return (
                <div key={p.user_id}
                  ...
                  <Button onClick={() => send(p)} disabled={sendingId === p.user_id}
