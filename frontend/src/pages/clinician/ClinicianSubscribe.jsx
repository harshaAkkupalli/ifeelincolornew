    ...
    try {
      ...
      await launchCheckout({
        ...
        onSuccess: async (data) => {
          ...
          setPaymentResult({
            presetKey: 'success',
            title: `Membership active — ${data.plan_name} 🩺`,
            message: 'Your full clinician toolkit is now unlocked. Welcome aboard.',
            primaryLabel: 'Open dashboard',
            onPrimary: () => { setPaymentResult(null); navigate('/clinician/home'); },
          });
        },
        onFailure: (err) => {
          setLoading(false);
          if (err?.dismissed) {
            setPaymentResult({
              presetKey: 'dismissed',
              onPrimary: () => { setPaymentResult(null); subscribe(); },
            });
          } else {
            setPaymentResult({
              presetKey: 'failure',
              message: err?.message
                ? `No funds were captured. (${err.message}) This is usually a transient gateway issue — please retry, or use a different card.`
                : undefined,
              onPrimary: () => { setPaymentResult(null); subscribe(); },
            });
          }
    ...
    <div className="px-4 pt-5 pb-6">
      ...
      <div className="mt-5 space-y-3">
        ...
        {plans.map((p, i) => {
          ...
          const isCurrent = !!(activeSub && activeSub.plan_id === p.plan_id);
          return (
            <div key={p.plan_id}
              data-testid={`clinician-plan-${p.plan_id}`}
              className={`w-full text-left rounded-xl p-5 relative transition bg-white border ${isCurrent ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-slate-200'}`}
              style={{ boxShadow: isCurrent ? '0 4px 12px rgba(31,111,84,0.12)' : '0 1px 2px rgba(16,24,40,0.06)' }}>
              {popular && (
              ...
              {/* Subscribe / Unsubscribe pair (per-card) */}
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button
                  data-testid={`clinician-subscribe-${p.plan_id}`}
                  disabled={isCurrent || loading}
                  onClick={() => { setSelected(p.plan_id); subscribe(); }}
                  className="rounded-lg py-2 text-xs font-semibold text-white flex items-center justify-center gap-1.5 transition"
                  style={{
                    background: isCurrent ? '#E2E8F0' : 'linear-gradient(135deg, #1F6F54, #2FA37A)',
                    color: isCurrent ? '#94A3B8' : 'white',
                    cursor: isCurrent ? 'not-allowed' : 'pointer',
                    opacity: isCurrent ? 0.7 : 1,
                  }}>
                  {free ? <><Sparkles className="w-3.5 h-3.5" /> Trial</> : <><Shield className="w-3.5 h-3.5" /> Subscribe</>}
                </button>
                <button
                  data-testid={`clinician-unsubscribe-${p.plan_id}`}
                  disabled={!isCurrent || loading}
                  onClick={async () => {
                    if (!isCurrent) return;
                    if (!window.confirm('Cancel your clinician membership? You will keep access until the current billing period ends.')) return;
                    try {
                      await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/me/subscription/cancel`, {}, { withCredentials: true });
                      setStatusMsg('Membership cancelled');
                      await load();
                      setTimeout(() => setStatusMsg(''), 2500);
                    } catch (e) {
                      setStatusMsg(e?.response?.data?.detail || 'Could not cancel');
                      setTimeout(() => setStatusMsg(''), 2500);
                    }
                  }}
                  className="rounded-lg py-2 text-xs font-semibold transition border"
                  style={{
                    background: isCurrent ? 'rgba(239,68,68,0.08)' : '#F1F5F9',
                    color: isCurrent ? '#DC2626' : '#94A3B8',
              ...
              {isCurrent && (
                <div className="absolute top-3 left-3 px-2 py-0.5 rounded-full bg-emerald-700 text-white text-[9px] font-bold uppercase tracking-wider">
                  Active
                </div>
              )}
            </div>
