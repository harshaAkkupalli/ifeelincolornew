...
import {
  ...
  DollarSign, FileText, X, Clock, Award, BookOpen, Megaphone,
} from 'lucide-react';
...
export default function ClinicianHomeMobile() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setAnnouncements((r.data.announcements || []).filter((a) => ['clinician', 'everyone'].includes(a.audience)).slice(0, 2));
    } catch { /* */ }
  };
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="px-4 pt-4 pb-6">
      ...
      <div className="grid grid-cols-3 gap-2.5 mt-4" data-testid="clinician-quick-actions">
        {[
          { to: '/clinician/recommendations', icon: BookOpen,   label: 'Library' },
      ...
      {announcements.length > 0 && (
        <div className="mt-6">
          ...
          <div className="space-y-2">
            {announcements.map((a) => (
              <motion.div key={a.announcement_id}
                whileHover={{ y: -2 }}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-fredoka font-semibold" style={{ color: C.ink }}>{a.title}</p>
                  <p className="text-xs font-nunito mt-0.5 line-clamp-2" style={{ color: 'rgba(10,42,32,0.62)' }}>{a.message}</p>
                </div>
              </motion.div>
            ))}
          </div>
