...
import { Megaphone, Plus, Trash2, Pencil, Image, Users, Stethoscope, Building2, ArrowRight } from 'lucide-react';
...
import { ANNOUNCEMENT_TARGETS } from '../../lib/announcementTargets';
...
export default function AnnouncementsPage() {
  ...
  const [form, setForm] = useState({ title: '', content: '', target_audience: 'all', image_data: '', link_target: '' });
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ title: '', content: '', target_audience: tab === 'all' ? 'all' : tab, image_data: '', link_target: '' });
    setShowForm(true);
  };
  const openEdit = (a) => {
    setEditItem(a);
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 mt-2">
            ...
            <Select
              ...
              onValueChange={(v) => setForm({ ...form, target_audience: v, link_target: '' })}
            >
              <SelectTrigger className="rounded-lg" data-testid="ann-audience-select"><SelectValue /></SelectTrigger>
              <SelectContent>{AUDIENCES.map(a => <SelectItem key={a.id} value={a.id}>{a.label}</SelectItem>)}</SelectContent>
            </Select>
            {/* Redirect target — depends on the role selected above */}
            <div>
              <label className="text-xs text-slate-500 mb-1 block flex items-center gap-1">
                <ArrowRight className="w-3 h-3 text-blue-500" />
                Open page on tap (optional)
              </label>
              <Select
                value={form.link_target || '__none__'}
                onValueChange={(v) => setForm({ ...form, link_target: v === '__none__' ? '' : v })}
              >
                <SelectTrigger className="rounded-lg" data-testid="ann-link-target-select">
                  <SelectValue placeholder="— None (no redirect)" />
                </SelectTrigger>
                <SelectContent>
                  {(ANNOUNCEMENT_TARGETS[form.target_audience] || ANNOUNCEMENT_TARGETS.all).map((t) => (
              ...
              {form.link_target && (
                <p className="text-[10px] text-slate-400 mt-1 truncate">Redirects to: {form.link_target}</p>
              )}
            </div>
            <div>
