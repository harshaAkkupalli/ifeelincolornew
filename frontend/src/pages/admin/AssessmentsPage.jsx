...
export default function AssessmentsPage() {
  ...
  // Category-level toggle (treatment_history & health_social)
  const [catSettings, setCatSettings] = useState({}); // {cat_id: {enabled, has_active_template}}
  const [needsSetupCat, setNeedsSetupCat] = useState(null);
  ...
  // Per-question edit state — when set, "Add Question" becomes "Update Question"
  ...
  const load = async () => {
    ...
    // Load category-level toggle settings (treatment_history + health_social)
    try {
      const cs = await ax.get(`${API}/assessment-categories`);
      const map = {};
      (cs.data.categories || []).forEach(c => { map[c.id] = c; });
      setCatSettings(map);
    } catch { /* ignore */ }
  };
  useEffect(() => { load(); }, [tab]); // eslint-disable-line

  const toggleCategory = async (catId, nextEnabled) => {
    try {
      const r = await ax.post(`${API}/assessment-categories/${catId}/toggle`, { enabled: nextEnabled });
      setCatSettings(s => ({ ...s, [catId]: { ...(s[catId] || {}), enabled: nextEnabled } }));
      if (nextEnabled && r.data.needs_setup) setNeedsSetupCat(catId);
  ...
  const addQuestion = () => {
    const q = {
      question_id: editingQIdx !== null
        ? form.questions[editingQIdx].question_id
        : `q_new_${Date.now()}`,
      ...
      image_url: qForm.image_url, accepted_types: qForm.accepted_types,
      order: editingQIdx !== null ? (form.questions[editingQIdx].order || editingQIdx + 1) : form.questions.length + 1,
    };
    setForm(f => {
      if (editingQIdx !== null) {
  ...
  const editQuestion = (idxOrQuestion, idxOverride = null) => {
    // Accept either an index (in-form edit) OR a question object + idx
    // (inline edit from a collapsed template, where form state is still
    // settling after openEdit). This avoids reading stale `form.questions`.
    const q = typeof idxOrQuestion === 'number'
      ? form.questions[idxOrQuestion]
      : idxOrQuestion;
    const targetIdx = typeof idxOrQuestion === 'number' ? idxOrQuestion : idxOverride;
    if (!q) return;
    setQForm({
      text: q.text || '',
      type: q.type || 'yes_no',
      ...
      scale_max_label: q.scale_max_label || 'Extremely',
      image_url: q.image_url || '',
      accepted_types: q.accepted_types || '.pdf,.jpg,.png',
    });
    setEditingQIdx(targetIdx);
  };
  ...
  const removeQuestion = (idx) => {
    if (editingQIdx === idx) cancelEditQuestion();
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value={tab}>
          {/* Per-category enable/disable toggle (super-admin) */}
          {(tab === 'treatment_history' || tab === 'health_social') && (
            <div
              className="flex items-center gap-3 bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-4"
              data-testid={`category-toggle-row-${tab}`}
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-800">
                  Show this section to patients
                </p>
                <p className="text-xs text-slate-500 mt-0.5">
                  When off, every patient skips <strong>{TYPES.find(t => t.id === tab)?.label}</strong> in their flow and continues with the other assessments.
                </p>
              </div>
              <Switch
                checked={catSettings[tab]?.enabled !== false}
                onCheckedChange={(v) => toggleCategory(tab, v)}
                data-testid={`category-toggle-${tab}`}
              />
              <span className="text-[10px] font-bold uppercase tracking-widest"
                style={{ color: catSettings[tab]?.enabled === false ? '#94A3B8' : '#22D67E' }}>
                {catSettings[tab]?.enabled === false ? 'Off' : 'On'}
              </span>
            </div>
          )}
                      ...
                      <div key={q.question_id} className="flex items-start gap-3 py-2 border-b border-slate-50 last:border-0">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-1.5 mt-1">
                            <span className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700">{Q_LABELS[q.type] || q.type}</span>
                        ...
                        {/* Inline per-question Edit — opens the full editor with this question loaded */}
                        <Button
                          variant="ghost" size="sm"
                          onClick={(e) => {
                            e.stopPropagation();
                            openEdit(a);
                            // Pass the actual question object so we don't depend
                            // on form state having settled yet.
                            setTimeout(() => editQuestion(q, i), 0);
                          }}
                          data-testid={`inline-edit-q-${a.template_id}-${i}`}
            ...
            <div className="space-y-4 mt-3">
              ...
              <div className="rounded-3xl p-5 relative overflow-hidden"
                ...
                {form.questions.map((q, i) => (
                  <div key={i}
                    className="flex items-center gap-2 py-2 px-3 rounded-xl bg-white/60 hover:bg-white/90 transition mb-1.5 shadow-sm"
                    style={editingQIdx === i ? { background: 'rgba(251,191,36,0.15)', border: '1px solid rgba(251,146,60,0.5)' } : {}}
                  >
                    ...
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700">{Q_LABELS[q.type]}</span>
                    <button
                      type="button"
                      onClick={() => editQuestion(i)}
                      data-testid={`edit-q-${i}`}
                      className="text-pink-500 p-1 hover:scale-110 transition"
                      title="Edit this question"
                    >
                    ...
                    <button
                      ...
                      data-testid={`remove-q-${i}`}
                      className="text-red-400 p-1 hover:scale-110 transition"
                    >
                ...
                <div className="mt-3 pt-3 border-t border-pink-100/60 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      className="rounded-2xl text-xs text-white border-0 flex-1 h-10 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition"
                      style={{
                        background: editingQIdx !== null
                          ? 'linear-gradient(135deg, #4EA8FF, #22D67E)'
                          : 'linear-gradient(135deg, #EC4899, #FB923C)',
                      }}
                    >
                      <Plus className="w-3.5 h-3.5 mr-1" />
                      {editingQIdx !== null ? `Update question ${editingQIdx + 1}` : 'Add Question'}
                    </Button>
                    {editingQIdx !== null && (
                      <Button
                        type="button"
                        ...
                        data-testid="cancel-edit-question-btn"
                        className="rounded-2xl text-xs h-10 bg-white/60 backdrop-blur border-slate-200"
                      >
                        Cancel
                      </Button>
                    )}
      ...
      {/* Needs-setup prompt — fires when admin enables a category that has no
          questions yet, asking them to add some. */}
      <Dialog open={!!needsSetupCat} onOpenChange={(v) => !v && setNeedsSetupCat(null)}>
        <DialogContent className="sm:max-w-md" data-testid="needs-setup-modal">
          <DialogHeader>
            <DialogTitle className="text-base font-fredoka">
              Add questions for {TYPES.find(t => t.id === needsSetupCat)?.label}?
            </DialogTitle>
          </DialogHeader>
          <p className="text-sm text-slate-600 leading-relaxed">
            You've turned this section on, but it doesn't have any questions yet — so patients won't see anything for it.
            Want to add questions now? You can also leave it for later — patients will simply skip the empty section.
          </p>
          <div className="flex justify-end gap-2 pt-3">
            <Button
              variant="outline"
              className="rounded-xl text-xs"
              onClick={() => setNeedsSetupCat(null)}
              data-testid="needs-setup-dismiss"
            >
              Later
            </Button>
            <Button
              ...
              data-testid="needs-setup-open-editor"
              onClick={() => {
                setTab(needsSetupCat);
                setNeedsSetupCat(null);
                openCreate();
              }}
            >
              <Plus className="w-3.5 h-3.5 mr-1" /> Add questions now
            </Button>
          </div>
        </DialogContent>
      </Dialog>
