...
# Logo lives under /app/backend/assets — embedded at cover + every page header.
LOGO_PATH = os.path.join(os.path.dirname(__file__), "assets", "ifc_logo.jpeg")
...
def _header(canv, doc):
    """Painted on every page: brand gradient bar + logo wordmark + footer."""
    canv.saveState()
    w, h = A4
    # Top gradient bar (solid rectangles approximate the brand gradient).
    canv.setFillColor(BRAND_PINK)
    canv.rect(0, h - 12, w * 0.33, 12, fill=1, stroke=0)
    canv.setFillColor(BRAND_VIOLET)
    canv.rect(w * 0.33, h - 12, w * 0.34, 12, fill=1, stroke=0)
    canv.setFillColor(BRAND_TEAL)
    canv.rect(w * 0.67, h - 12, w * 0.33, 12, fill=1, stroke=0)
    # Logo + wordmark (skip on page 1 — the cover renders the big logo)
    if doc.page > 1 and os.path.exists(LOGO_PATH):
        try:
            canv.drawImage(
                LOGO_PATH, 18, h - 32 - 16, width=16, height=16,
        ...
        canv.drawString(40, h - 32 - 4, "IFEELINCOLOR")
        canv.setFont("Helvetica", 7.5)
        canv.setFillColor(MUTED)
        canv.drawRightString(w - 18, h - 32 - 4, "Patient Wellbeing Report")
    # Footer
    canv.setFillColor(MUTED)
    canv.setFont("Helvetica", 8)
    canv.drawCentredString(
        w / 2, 18,
        "IFEELINCOLOR · Confidential health dossier · Developed by Projexino Solutions Pvt Ltd",
    )
...
def build_dossier_pdf(
    ...
    # ── Cover ────────────────────────────────────────────────────────────
    # Centered logo on top, large brand title, subtitle, generated-on date,
    # and a horizontal "Generated for <Patient>" tag.
    if os.path.exists(LOGO_PATH):
        try:
            logo = Image(LOGO_PATH, width=42 * mm, height=42 * mm, kind="proportional")
            logo.hAlign = "CENTER"
    ...
    cover_title_style = ParagraphStyle(
        "ctitle", fontSize=28, leading=32, alignment=1,
        textColor=INK, fontName="Helvetica-Bold", spaceAfter=2,
    )
    cover_subtitle_style = ParagraphStyle(
        "csub", fontSize=12, leading=15, alignment=1,
        textColor=BRAND_VIOLET, fontName="Helvetica-Bold", spaceAfter=2,
    )
    cover_meta_style = ParagraphStyle(
        "cmeta", fontSize=9, leading=12, alignment=1,
        textColor=MUTED, fontName="Helvetica",
    )
    story.append(Paragraph("IFEELINCOLOR", cover_title_style))
    story.append(Paragraph("Patient Assessment &amp; Wellbeing Report", cover_subtitle_style))
    story.append(Spacer(1, 8))
    today = datetime.utcnow().strftime("%A, %d %B %Y")
    story.append(Paragraph(
        f"Generated for <font color='{INK.hexval()}'><b>{_escape(patient.get('name') or '—')}</b></font>"
        f" &nbsp;·&nbsp; {today}",
        cover_meta_style,
    ))

    # Brand chip strip (visual accent below the cover)
    chip_table = Table(
        [[
            Paragraph(
                '<font name="Helvetica-Bold" size="7.5" color="white">CONFIDENTIAL</font>',
                ParagraphStyle("cp", alignment=1, textColor=colors.white),
            ),
            Paragraph(
                '<font name="Helvetica-Bold" size="7.5" color="white">EVIDENCE-INFORMED</font>',
                ParagraphStyle("cp", alignment=1, textColor=colors.white),
            ),
            Paragraph(
                '<font name="Helvetica-Bold" size="7.5" color="white">PATIENT-CENTRED</font>',
                ParagraphStyle("cp", alignment=1, textColor=colors.white),
            ),
        ]],
    ...
    chip_table.setStyle(TableStyle([
        ...
        ("BACKGROUND", (1, 0), (1, 0), BRAND_VIOLET),
