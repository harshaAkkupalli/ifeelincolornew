...
class PlanCreate(BaseModel):
    ...
    trial_days: int = 0
    features: List[str] = []
    # Audience-aware fields — required so the patient-facing
    # `resolve_plans_for_clinician(clinician_id)` resolver can find the
    # admin's per-clinician plans. Without these, the plan is saved with
    # `audience` and `clinician_id` stripped by Pydantic → the patient
    # subscribe page falls back to global plans and the per-clinician
    # plan never shows up.
    audience: Optional[str] = None  # patient_clinician | patient_organization | patient_portal | clinician_portal | organization_portal
    clinician_id: Optional[str] = None  # required when audience == 'patient_clinician'
    org_id: Optional[str] = None        # required when audience == 'patient_organization'
...
async def list_clinicians(request: Request, search: str = "", skip: int = 0, limit: int = 50):
    ...
    for c in clinicians:
        ...
        c["subscription"] = sub
        # Assigned patient count + small preview list. MUST use the same union
        # as the clinician's own dashboard (`_clinician_patient_ids` in
        # server.py) so admin and clinician views report the SAME number.
        cid = c["user_id"]
        seen_ids: dict = {}
        async for s in db.patient_doctor_subscriptions.find(
            {"$or": [{"clinician_id": cid}, {"doctor_id": cid}]},
        ...
        async for s in db.patient_subscriptions.find(
            {"clinician_id": cid, "status": "active"},
            {"_id": 0, "user_id": 1, "subscribed_at": 1, "end_date": 1},
        ):
            pid = s.get("user_id")
            if pid and pid not in seen_ids:
        ...
        async for s in db.doctor_subscriptions.find(
            ...
            if pid and pid not in seen_ids:
                seen_ids[pid] = s
        # Drop stale ids that don't resolve to a current patient user — same
        # rule as `_clinician_patient_ids` so admin & clinician views agree.
        valid_ids: set = set()
        if seen_ids:
            async for u in db.users.find(
                {"role": "patient", "user_id": {"$in": list(seen_ids.keys())}},
                {"_id": 0, "user_id": 1},
            ):
...
async def update_plan(plan_id: str, request: Request):
    await require_super_admin(request, db)
    body = await request.json()
    allowed = ["name", "description", "price", "price_usd", "duration_days", "plan_type", "is_trial", "trial_days", "features", "status", "active", "purpose", "color", "order", "audience", "clinician_id", "org_id"]
