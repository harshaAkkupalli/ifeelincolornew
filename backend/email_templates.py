...
@email_router.get("/email-templates/ai-status")
async def ai_status(request: Request):
    """Frontend-friendly probe: returns whether AI generation is currently
    available + which provider would be used. Admins can hit this from the
    UI to diagnose the "AI email template creation not working" symptom
    immediately, without trial-and-error.
    """
    await get_current_admin(request, db)
    from llm_adapter import has_llm_key, active_provider
    ready = has_llm_key()
    provider = active_provider() if ready else None
    return {
        "ready": ready,
        "provider": provider,
        "hint": (
            None if ready else
            "Set GOOGLE_GENAI_API_KEY (easiest — free tier at "
            "https://aistudio.google.com/apikey) OR OPENAI_API_KEY "
            "OR ANTHROPIC_API_KEY OR EMERGENT_LLM_KEY in the backend "
            "environment and restart to enable AI."
        ),
    }
...
async def _llm_email_call(system_message: str, user_text: str) -> dict:
    """Wrap the LLM adapter with diagnostics tailored for the Email-Templates
    UI. The user reported "AI email template creation not working after
    hosting our own domain" — usually because no LLM key is configured on
    the self-host. We surface a clear, actionable 400 in that case and a
    detailed 502 (with the upstream error tail) when the key IS present
    but the vendor call fails, so the admin can fix it quickly.
    """
    from llm_adapter import call_llm, has_llm_key, active_provider
    if not has_llm_key():
        raise HTTPException(
            status_code=400,
            detail=(
                "AI email generation is unavailable because no LLM provider "
                "key is configured. Set ONE of the following environment "
                "variables on your server and restart the backend:\n"
                "  • GOOGLE_GENAI_API_KEY (easiest — free tier at "
                "https://aistudio.google.com/apikey)\n"
                "  • OPENAI_API_KEY (most reliable, paid)\n"
                "  • ANTHROPIC_API_KEY\n"
    ...
    try:
        ...
        logger.exception("AI email template call failed via provider=%s", provider)
        # Surface the upstream message tail so the admin can diagnose
        # (rate-limit / invalid key / model-not-found etc.) without SSHing.
        tail = str(e).splitlines()[-1] if str(e) else "unknown error"
        raise HTTPException(
            status_code=502,
            detail=(
                f"AI email generation failed via provider '{provider}'. "
                f"Upstream error: {tail[:300]}. "
                "Common causes: invalid API key, expired credit, model name "
                "not allowed for your key, or temporary vendor outage."
            ),
        )
