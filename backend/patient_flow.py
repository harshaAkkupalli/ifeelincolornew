...
async def list_plans(role: str = "patient"):
    """Return subscription plans visible to a given role.

    Audience matrix:
      role=patient       → patient_portal       (patient pays IFEELINCOLOR)
      role=clinician     → clinician_portal     (clinician pays IFEELINCOLOR)
      role=organization  → organization_portal  (org pays IFEELINCOLOR)

    We FIRST try the audience-aware key (new admin UI writes it) and ONLY
    fall back to the legacy `plan_type` filter for plans that don't carry
    an `audience` yet — and we explicitly EXCLUDE patient_clinician /
    patient_organization plans so the clinician portal never shows
    ...
    if role == "patient":
        ...
        exclude_audiences = ["patient_clinician", "patient_organization", "clinician_portal", "organization_portal"]
    elif role == "clinician":
        audience = "clinician_portal"
        # Legacy: plan_type='clinician' WITHOUT audience=patient_clinician
        legacy_filter = {"plan_type": "clinician"}
        exclude_audiences = ["patient_clinician", "patient_portal", "patient_organization", "organization_portal"]
    elif role == "organization":
        audience = "organization_portal"
        legacy_filter = {"plan_type": "organization"}
        exclude_audiences = ["patient_clinician", "patient_portal", "patient_organization", "clinician_portal"]
    else:
        audience = role
        legacy_filter = {"plan_type": role}
        exclude_audiences = []
            ...
            "$and": [
                {
                    "$or": [
                        {"audience": audience},
                        # Legacy plans (no audience set) — fall back to plan_type match
                        {"$and": [
                            legacy_filter,
                            {"audience": {"$in": [None, ""]}}
                        ]},
                ...
                {"audience": {"$nin": exclude_audiences}},
                {"$or": [
                    {"active": True},
                    {"status": "active"},
                ]},
...
async def subscribe_mock(payload: SubscribeRequest, request: Request):
    ...
    # ── Defense-in-depth — block paid-plan bypass ──────────────────────
    # `/subscribe/mock` is for FREE trials only. Paid plans MUST flow through
    # Razorpay (/api/razorpay/order → /verify). Refuse here so a buggy
    # frontend or direct API call cannot silently grant a paid subscription.
    price = float(plan.get("price_usd") or plan.get("price") or 0)
    if price > 0:
        raise HTTPException(
            status_code=402,
            detail="This is a paid plan — please complete checkout via Razorpay.",
        )
...
@patient_flow_router.post("/me/subscription/cancel")
async def cancel_my_portal_subscription(request: Request):
    """Cancels the current user's portal subscription (Patient / Clinician /
    Organization). Used by the Unsubscribe button on every portal-plan card.

    Marks the active sub as `cancelled`. If a future-dated `end_date` exists
    we keep it (so the user retains access until the period ends — standard
    Stripe / Razorpay behaviour); otherwise we cancel immediately."""
    user = await _current_user(request)
    role = (user.get("role") or "patient").lower()
    coll_map = {
        "clinician": db.clinician_subscriptions,
    ...
    coll = coll_map.get(role, db.patient_subscriptions)
    sub = await coll.find_one(
        {"user_id": user["user_id"], "status": {"$in": ["active", "trial", "trialing"]}},
        sort=[("created_at", -1)],
    )
    if not sub:
        return {"ok": True, "already": True, "message": "No active subscription to cancel."}
    await coll.update_one(
        {"subscription_id": sub["subscription_id"]},
        {"$set": {"status": "cancelled", "cancelled_at": _now_iso()}},
    )
    return {"ok": True, "subscription_id": sub["subscription_id"]}
...
async def _category_enabled(category_id: str) -> bool:
    """Returns True if this assessment category is enabled for patients.
    Admin can toggle treatment_history & health_social on/off; `assessment`
    is always on (it's the final paid step)."""
    if category_id == "assessment":
        return True
    s = await db.assessment_category_settings.find_one(
...
async def assessment_categories():
    """Return categories with their enabled flag so the patient hub can hide
    admin-disabled categories and skip them in the flow."""
    out = []
    for c in ASSESSMENT_CATEGORIES:
        out.append({**c, "enabled": await _category_enabled(c["id"])})
...
async def patient_assessment_questions(category_id: str, request: Request):
    ...
    If the admin has DISABLED the category, return source='disabled' so the patient
    hub can skip it and continue the flow with the remaining categories."""
    await _current_user(request)  # auth gate
    # Honor the admin's category-level toggle.
    if not await _category_enabled(category_id):
        return {
            "category_id": category_id,
            "questions": [],
            "source": "disabled",
            "message": "This section has been turned off by the administrator. You can continue with the rest of your assessment.",
        }
    type_map = {
...
# ── Patient → Clinician ratings & feedback (1-5 stars + free-text) ──
class ClinicianRatingPayload(BaseModel):
    rating: int  # 1-5
    feedback: str = ""


@patient_flow_router.put("/patient/doctors/{doctor_id}/rating")
async def upsert_clinician_rating(doctor_id: str, payload: ClinicianRatingPayload, request: Request):
    """Idempotent upsert of the patient's rating + feedback for a clinician
    they actively subscribe to. Stored in `clinician_ratings` keyed by
    (user_id, doctor_id). Recomputes the clinician's average + count for
    fast read on the patient's home + clinician profile.
    """
    user = await _current_user(request)
    if payload.rating < 1 or payload.rating > 5:
        raise HTTPException(400, "rating must be 1..5")
    sub = await db.patient_doctor_subscriptions.find_one(
        {"user_id": user["user_id"], "doctor_id": doctor_id}, {"_id": 0}
    )
    if not sub:
        raise HTTPException(403, "Subscribe to this clinician before rating")
    now = _now_iso()
    await db.clinician_ratings.update_one(
        {"user_id": user["user_id"], "doctor_id": doctor_id},
        {"$set": {
            "user_id": user["user_id"],
            "doctor_id": doctor_id,
    ...
    pipeline = [
        ...
        {"$group": {"_id": "$doctor_id", "avg": {"$avg": "$rating"}, "n": {"$sum": 1}}},
    ]
    agg = await db.clinician_ratings.aggregate(pipeline).to_list(1)
    avg = round(agg[0]["avg"], 2) if agg else None
    count = agg[0]["n"] if agg else 0
    if avg is not None:
        await db.users.update_one(
            {"user_id": doctor_id, "role": "clinician"},
            {"$set": {"clinician_info.avg_rating": avg, "clinician_info.rating_count": count}},
        )
    return {"ok": True, "rating": payload.rating, "feedback": payload.feedback, "clinic_avg": avg, "rating_count": count}


@patient_flow_router.get("/patient/doctors/{doctor_id}/rating")
async def get_my_clinician_rating(doctor_id: str, request: Request):
    user = await _current_user(request)
    row = await db.clinician_ratings.find_one(
        {"user_id": user["user_id"], "doctor_id": doctor_id}, {"_id": 0}
    )
    return {"rating": (row or {}).get("rating"), "feedback": (row or {}).get("feedback", "")}
...
async def list_clinician_plans():
    """Returns all admin-curated clinician plans. Looks in BOTH the
    audience-aware `db.subscription_plans` (where the modern PlanManagement
    UI writes) and the legacy `db.plans` collection."""
    plans: list = []
    # Audience-aware store (preferred)
    ...
    async for p in db.plans.find(
        {"plan_type": "clinician", "is_trial": {"$ne": True}},
        {"_id": 0},
    ).sort("price", 1):
        if not any((q.get("plan_id") == p.get("plan_id")) for q in plans):
            plans.append(p)
    # Hard-coded fallback if admin hasn't curated any yet
    if not plans:
        plans = [
...
# ── Discover all clinicians (masked) for the patient "All Clinicians" tab ──
@patient_flow_router.get("/clinicians/discover")
async def discover_all_clinicians(request: Request, q: Optional[str] = None):
    """List every active clinician account (free trial + paid). Returns only
    the public-safe fields: name, profile_photo, years_experience, specialty,
    and an is_subscribed_by_me flag. Bio / phone / address remain masked
    unless the patient has an active patient → clinician subscription.

    The `q` parameter filters by clinician name (case-insensitive) — used by
    the autocomplete search on the Patient Subscribe → Clinician Plans tab.
    """
    user = await _current_user(request)
    cursor = db.users.find({"role": "clinician"}, {"_id": 0, "password_hash": 0})
    clinicians = await cursor.to_list(500)
    # My active subscriptions to clinicians
    my_subs = await db.patient_doctor_subscriptions.find(
        {"user_id": user["user_id"]}, {"_id": 0, "doctor_id": 1}
    ).to_list(500)
    subbed_ids = {s["doctor_id"] for s in my_subs}
    qlower = (q or "").strip().lower()
    out = []
    for c in clinicians:
        info = c.get("clinician_info") or {}
        if qlower and qlower not in (c.get("name") or "").lower():
            continue
        is_subbed = c["user_id"] in subbed_ids
        out.append({
            "clinician_id": c["user_id"],
            "name": c.get("name") or "Clinician",
            "profile_photo": info.get("profile_photo") or c.get("avatar_url"),
            "years_experience": info.get("years_experience") or info.get("years_of_experience"),
            "specialty": info.get("specialization") or info.get("specialty") or "Mental Health Specialist",
            "subscription_tier": (c.get("subscription") or {}).get("plan_name") or "Free trial",
            "is_subscribed_by_me": is_subbed,
            # Masked fields exposed only when subscribed
            "bio": info.get("bio") if is_subbed else None,
            "phone": c.get("phone") if is_subbed else None,
            "address": info.get("address") if is_subbed else None,
            "email": c.get("email") if is_subbed else None,
            "languages": info.get("languages") if is_subbed else None,
        })
    return {"clinicians": out, "count": len(out)}


# ── Clinician's masked patient-discovery feed ──
@patient_flow_router.get("/clinician/discover-patients")
async def clinician_discover_patients(request: Request):
    """HIPAA-aware discoverable-patient feed.

    A clinician is **only** allowed to see identifiable patient information
    (name, profile photo, exact city, email, phone, age, primary concerns)
    once a provider–patient relationship exists — i.e. the patient appears
    in this clinician's authorised set (subscribed via any of the three
    subscription collections OR an explicit `clinician_assignments` row).

    For every other patient on the platform we return ONLY:
      • a stable de-identified handle  ("Patient #A1B2")
      • the region (state/country), never the city
      • a hash-derived avatar background — never the real photo
      • `is_unlocked: false`

    This way a clinician can still browse the size of the patient pool
    on the platform (so they understand reach) without learning who the
    ...
    # Canonical authorised set — same union the rest of the platform uses
    # (subscription + assignment). `_clinician_patient_ids` already filters
    # to real patient user_ids so we don't surface PHI for stale rows.
    from server import _clinician_patient_ids  # noqa: WPS433 — late import to avoid cycle
    authorised: set = await _clinician_patient_ids(user["user_id"])
    # Also honour the explicit clinician_assignments collection (used when
    # an org pairs a patient with a clinician without going through the
    # subscription flow).
    async for a in db.clinician_assignments.find(
        {"clinician_id": user["user_id"], "status": "active"},
        {"_id": 0, "patient_id": 1},
    ):
        if a.get("patient_id"):
            authorised.add(a["patient_id"])

    cursor = db.users.find({"role": "patient"}, {"_id": 0, "password_hash": 0})
    out = []
    for p in await cursor.to_list(2000):
        pid = p["user_id"]
        is_unlocked = pid in authorised
        info = p.get("patient_info") or {}
        loc = info.get("location") or {}
        if is_unlocked:
            # Full provider–patient view
            out.append({
                "patient_id": pid,
                "name": p.get("name") or "Patient",
                "profile_photo": info.get("profile_photo") or p.get("avatar_url"),
                "city": loc.get("city") or loc.get("region") or "—",
                "is_unlocked": True,
                "email": p.get("email"),
                "phone": p.get("phone"),
                "age": info.get("age"),
                "primary_concerns": info.get("primary_concerns"),
            })
        else:
            # De-identified view — never expose the real name or photo.
            # Stable handle derived from a SHA-256 prefix of the user_id so
            # the same patient always shows the same handle to the same
            # clinician (helpful when reading dashboards over time) but
            # the handle itself contains no PHI and is irreversible.
            digest = hashlib.sha256(pid.encode("utf-8")).hexdigest()[:4].upper()
            out.append({
                "patient_id": pid,
                "name": f"Patient #{digest}",
                "profile_photo": None,
                # Only show region/country granularity for non-authorised
                # patients — never the city.
                "city": loc.get("region") or loc.get("country") or "—",
                "is_unlocked": False,
                "email": None,
                "phone": None,
                "age": None,
    ...
    return {"patients": out, "count": len(out), "unlocked_count": len(authorised)}
...
async def subscribe_to_doctor_paid(payload: DoctorSubscribePaid, request: Request):
    ...
    Resolves `plan_id` against EVERY plan store the rest of the platform
    uses so that any admin-created plan from `/admin/plans` works here:
      • legacy `db.plans` with `plan_type='clinician'` (fallback defaults)
      • `db.subscription_plans` — where the audience-aware admin UI writes
        every plan (audience='patient_clinician' for per-clinician plans,
        possibly with a `clinician_id` for clinician-specific pricing).
    """
    user = await _current_user(request)
    plan = None
    # 1) legacy clinician-plan list (3 hard-coded fallbacks)
    legacy = await list_clinician_plans()
    plan = next((p for p in legacy["plans"] if p["plan_id"] == payload.plan_id), None)
    # 2) The collection where the admin UI actually persists plans.
    if not plan:
        doc = await db.subscription_plans.find_one({"plan_id": payload.plan_id}, {"_id": 0})
        if doc:
            # Either it's audience=patient_clinician, or it's a legacy
            # plan_type='clinician'. Either way it's a clinician plan.
            is_clin = (
                doc.get("audience") == "patient_clinician"
                or doc.get("plan_type") == "clinician"
            )
            if not is_clin and not doc.get("audience") and not doc.get("plan_type"):
                # Untagged admin plan — accept it (covers older seeds).
                is_clin = True
            if is_clin:
                plan = {
                    "plan_id": doc["plan_id"],
                    "name": doc.get("name") or doc.get("plan_name") or "Care plan",
                    "price": float(doc.get("price") or doc.get("price_usd") or 0),
                    "duration_days": int(doc.get("duration_days") or 30),
                }
    # 3) Last-resort: legacy `db.plans` collection (kept for backward-compat
    #    with very old installs).
    if not plan:
        doc = await db.plans.find_one({"plan_id": payload.plan_id}, {"_id": 0})
        if doc and (doc.get("audience") in (None, "patient_clinician") or doc.get("plan_type") == "clinician"):
            plan = {
                "plan_id": doc["plan_id"],
                "name": doc.get("name") or doc.get("plan_name") or "Care plan",
                "price": float(doc.get("price") or doc.get("price_usd") or 0),
    ...
    # ── Defense-in-depth — block paid-plan bypass ──────────────────────
    # This endpoint is the "free trial / instant activation" route. If the
    # resolved plan has a price > 0 the patient MUST go through Razorpay
    # (POST /api/razorpay/order → /api/razorpay/verify). Refuse to fulfil
    # paid plans here so a buggy frontend OR a direct API call cannot
    # silently grant a paid clinician subscription for free.
    ...
    rec = {
        "subscription_id": f"docsub_{uuid.uuid4().hex[:10]}",
        "user_id": user["user_id"],
        # Keep both keys so the celebration feed (which queries by
        # clinician_id OR doctor_id) reliably matches every record.
...
async def trigger_emergency(payload: EmergencyTrigger, request: Request):
    """Patient SOS dispatch.

    Per the 2026-06-04 user directive: alerts are broadcast to
      (a) every clinician the patient subscribes to,
      (b) every clinician within 50 km of the patient (if geo provided),
      (c) every admin in the system.
    Each recipient gets a row in `emergency_dispatches` with `ack=false`,
    so the clinician/admin UI can poll `/api/emergency/active` every 5 s
    and trigger an audible alarm + popup the moment the row is delivered.
    """
    user = await _current_user(request)
    lat = payload.lat if payload.lat is not None else 40.7128
    lng = payload.lng if payload.lng is not None else -74.0060
    # Static mock dispatch (kept for backwards compatibility of legacy clients)
    ...
    recipient_kinds: dict = {}
    # (a) Subscribed clinicians
    async for s in db.patient_doctor_subscriptions.find(
        {"user_id": user["user_id"]}, {"_id": 0, "clinician_id": 1, "doctor_id": 1}
    ):
        cid = s.get("clinician_id") or s.get("doctor_id")
        if cid:
            recipient_ids.add(cid)
            recipient_kinds[cid] = "subscribed_clinician"
    # (b) Nearby clinicians (within 50 km) — requires lat/lng on the clinician
    if payload.lat is not None and payload.lng is not None:
        async for c in db.users.find(
            {"role": "clinician"},
            {"_id": 0, "user_id": 1, "clinician_info": 1, "lat": 1, "lng": 1},
        ):
            clat = c.get("lat") or (c.get("clinician_info") or {}).get("lat")
            clng = c.get("lng") or (c.get("clinician_info") or {}).get("lng")
            if clat is None or clng is None:
                continue
            try:
                if _haversine_km(lat, lng, float(clat), float(clng)) <= 50:
                    cid = c["user_id"]
                    recipient_ids.add(cid)
                    recipient_kinds.setdefault(cid, "nearby_clinician")
            except Exception:
                continue
    # (c) Admins — fetch from db.admins collection
    admin_ids: list = []
    async for a in db.admins.find({}, {"_id": 0, "admin_id": 1}):
        if a.get("admin_id"):
            admin_ids.append(a["admin_id"])
    ...
    alert = {
        ...
        "lat": lat,
        "lng": lng,
        "dispatched_to_doctors": list(recipient_ids),
        "dispatched_to_admins": admin_ids,
    ...
    alert.pop("_id", None)
    # Per-recipient dispatches so the clinician/admin UI can show "Acknowledge"
    dispatch_docs = []
    for cid in recipient_ids:
        dispatch_docs.append({
            ...
            "recipient_kind": recipient_kinds.get(cid, "subscribed_clinician"),
            "ack": False,
            "created_at": now_iso,
        })
    ...
    if dispatch_docs:
        await db.emergency_dispatches.insert_many(dispatch_docs)
    return {
        "alert": alert,
        "dispatched": {
            "doctors": [{"name": d["name"], "phone": "+1-555-1000", "specialty": d["specialty"]} for d in nearest_docs],
            "police": police,
            "subscribed_clinician_count": sum(1 for k in recipient_kinds.values() if k == "subscribed_clinician"),
            "nearby_clinician_count": sum(1 for k in recipient_kinds.values() if k == "nearby_clinician"),
...
# ── Active SOS feed (clinician + admin) ──
@patient_flow_router.get("/emergency/active")
async def list_active_emergency(request: Request):
    """Returns un-acknowledged emergency dispatches for the calling user.

    Polled every 5 s by the Clinician Shell + Admin Shell. When a fresh
    row appears, the UI plays an audible alarm and pops a full-screen
    modal with the patient's name + phone until the recipient taps
    'Acknowledge'.
    """
    # Resolve calling identity (clinician via session_token; admin via admin_token)
    recipient_id = None
    role = None
    ...
    if token:
        ...
        if sess:
            u = await db.users.find_one({"user_id": sess["user_id"]}, {"_id": 0, "role": 1, "user_id": 1})
            if u and u.get("role") == "clinician":
                recipient_id = u["user_id"]
                role = "clinician"
    if not recipient_id:
        admin_tok = request.cookies.get("admin_token")
        if admin_tok:
            a_sess = await db.admin_sessions.find_one({"token": admin_tok}, {"_id": 0})
            if a_sess:
                admin = await db.admins.find_one({"admin_id": a_sess["admin_id"]}, {"_id": 0})
                if admin:
                    recipient_id = admin["admin_id"]
                    role = "admin"
    if not recipient_id:
        return {"active": [], "server_time": _now_iso()}

    rows = await db.emergency_dispatches.find(
        {"recipient_id": recipient_id, "ack": False}, {"_id": 0}
    ).sort("created_at", -1).to_list(20)
    # Hydrate each row with the alert + patient details
    out = []
    for r in rows:
        alert = await db.emergency_alerts.find_one(
            {"alert_id": r["alert_id"]}, {"_id": 0}
        )
        if not alert:
            continue
        patient = await db.users.find_one(
            {"user_id": alert.get("user_id")},
            {"_id": 0, "name": 1, "phone": 1, "mobile": 1, "email": 1, "profile_photo": 1},
        )
        last_checkin = await db.patient_checkins.find_one(
            {"$or": [{"patient_id": alert.get("user_id")}, {"user_id": alert.get("user_id")}]},
            {"_id": 0, "user_selected_emotion": 1, "user_selected_color": 1, "created_at": 1},
            sort=[("created_at", -1)],
        )
        out.append({
            "dispatch_id": r["dispatch_id"],
            "alert_id": r["alert_id"],
            "kind": r.get("recipient_kind"),
            ...
            "patient": {
                "user_id": alert.get("user_id"),
                "name": (patient or {}).get("name") or alert.get("user_name"),
                "phone": (patient or {}).get("phone") or (patient or {}).get("mobile") or alert.get("user_phone") or "",
                "email": (patient or {}).get("email") or alert.get("user_email"),
                "profile_photo": (patient or {}).get("profile_photo"),
                "last_emotion": (last_checkin or {}).get("user_selected_emotion"),
                "last_color": (last_checkin or {}).get("user_selected_color"),
            },
        })
    return {"active": out, "server_time": _now_iso(), "role": role}


@patient_flow_router.post("/emergency/dispatch/{dispatch_id}/ack")
async def ack_emergency(dispatch_id: str, request: Request):
    """Recipient acknowledges an SOS — stops the alarm + popup on their side."""
    # Identify the caller — clinician OR admin (same dual cookie pattern)
    recipient_id = None
    token = request.cookies.get("session_token")
    if token:
        sess = await db.user_sessions.find_one({"session_token": token}, {"_id": 0})
        if sess:
            recipient_id = sess.get("user_id")
    if not recipient_id:
        admin_tok = request.cookies.get("admin_token")
        if admin_tok:
            a_sess = await db.admin_sessions.find_one({"token": admin_tok}, {"_id": 0})
            if a_sess:
                recipient_id = a_sess.get("admin_id")
    if not recipient_id:
        raise HTTPException(401, "Not authenticated")
    res = await db.emergency_dispatches.update_one(
        {"dispatch_id": dispatch_id, "recipient_id": recipient_id},
        {"$set": {"ack": True, "ack_at": _now_iso()}},
    )
