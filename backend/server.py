...
async def clinician_dashboard_stats(request: Request):
    ...
    # Subscribed patient IDs
    subs = await db.patient_subscriptions.find(
        {"clinician_id": clinician_id, "status": "active"}, {"_id": 0}
    ).to_list(1000)
    subscribed_patient_ids = list({s.get("user_id") for s in subs if s.get("user_id")})
    # Fallback: any patient with a doctor relationship
    rel_patient_ids = []
    async for rel in db.doctor_subscriptions.find({"clinician_id": clinician_id}, {"_id": 0}):
        rel_patient_ids.append(rel.get("user_id"))
    # NEW: patient_doctor_subscriptions where clinician_id = this clinician
    ...
    all_patient_ids = list({*subscribed_patient_ids, *rel_patient_ids, *pds_patient_ids})
    # PHI guard — only show patients who explicitly subscribed to this
    # clinician. NEVER fall back to a global patient list (was leaking PHI
    # to fresh clinicians with zero caseload).
    if all_patient_ids:
        patients = await db.users.find(
    ...
    return {
        ...
        "subscribed_count": len({*subscribed_patient_ids, *pds_patient_ids}),
