...
async def org_list_clinicians(request: Request):
    ...
    # Lightweight stats: patient count via subscriptions
    out = []
    for c in rows:
        pds_count = await db.patient_doctor_subscriptions.count_documents({"clinician_id": c["user_id"]})
        out.append({**c, "patient_count": pds_count})
